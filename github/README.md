# kiyocode GitHub Action

A GitHub Action that integrates [kiyocode](https://kiyocode.ai) directly into your GitHub workflow.

Mention `/opencode` in your comment, and kiyocode will execute tasks within your GitHub Actions runner.
...
Leave the following comment on a GitHub issue. kiyocode will create a new branch, implement the changes, and open a PR with the changes.
...
Leave the following comment on a GitHub PR. kiyocode will implement the requested change and commit it to the same PR.
...
Leave a comment directly on code lines in the PR's "Files" tab. kiyocode will automatically detect the file, line numbers, and diff context to provide precise responses.
...
When commenting on specific lines, kiyocode receives:
   ...
   jobs:
     opencode:
       ...
       steps:
          ...
          - name: Run opencode
           uses: dedeprogames-official/kiyocode/github@latest
           env:
...
This is an early release. If you encounter issues or have feedback, please create an issue at https://github.com/dedeprogames-official/kiyocode/issues.
...
2. Run:
   ...
   - `MODEL`: The model used by kiyocode. Same as the `MODEL` defined in the GitHub workflow.
   ...
   - `/path/to/opencode`: Path to your cloned kiyocode repo. `bun /path/to/opencode/github/index.ts` runs your local version of `opencode`.
