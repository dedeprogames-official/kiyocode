...
Move the canonical KiyoCode terminal application from
...
The SDK is the TUI's KiyoCode boundary. Missing backend data or operations must
...
- KiyoCode domain data displayed by the TUI
...
route, and feature-plugin code no longer reads KiyoCode globals or process state;
...
Replace process-global KiyoCode state with resolved TUI inputs.
...
adapters rather than KiyoCode domain implementations.

Make the SDK the only KiyoCode domain boundary used by the TUI.
...
- Domain-facing TUI code imports KiyoCode data and operations only from
  `@opencode-ai/sdk`.
...
- TUI package imports do not reach into executable or backend implementation
  packages.
- SDK wire data is treated as the source of truth for KiyoCode domain state.
