...
Create a small workspace package that vendors the Drizzle `effect-sqlite` adapter shape for our repo. This is not an kiyocode storage abstraction. It is a local package that ports the Drizzle Effect SQLite implementation so we can use it before/independently of upstream release timing.

`packages/opencode` will use it internally, but the package itself should be generic: Drizzle + Effect + SQLite. No kiyocode paths, migrations, tables, transaction hooks, post-commit behavior, or domain language should live in this package.
...
- Kiyocode-specific path/channel selection stays in `packages/opencode`.

## Kiyocode Adoption Notes

These are not package requirements, but they matter for the later kiyocode adoption PR.

The current `packages/opencode/src/storage/db.ts` has two non-obvious semantics that the kiyocode wrapper must preserve when it consumes this adapter:
...
The kiyocode wrapper can implement that using Effect context instead of `LocalContext`:
...
Do not remove this behavior while moving kiyocode to Effect SQLite. `SyncEvent.run` depends on transaction composability and `behavior: "immediate"` for sequencing correctness.
...
8. Only then build domain stores like session/message/project stores on top of kiyocode's storage wrapper.
...
An Effect Drizzle SQLite package lets us vendor the adapter once. Then kiyocode can build its own storage wrapper on top, and `SessionStorage`, `MessageStorage`, event store, and projector writes can all share the same transaction and migration model.
...
- Use a tiny test schema, not kiyocode domain tables.
...
That gives us a focused place to validate the Effect SQLite approach before disturbing kiyocode's current database runtime.
