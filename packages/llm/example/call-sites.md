...
Kit and Aidan want provider-specific LLM behavior to move out of kiyocode's AI
...
kiyocode's dynamic runtime should construct executable models at its app
...
- Effect AI: executable models carry provider requirements and can be resolved by
  an app boundary.
- LiteLLM/kiyocode config: dynamic `providerID/modelID` branching belongs at the
...
- [ ] Update tests so direct route/provider tests assert route values are carried
      by executable models, and kiyocode/native tests assert boundary-based route
      selection.
