---
title: Providers
description: Using any LLM provider in KiyoCode.
---

import config from "../../../config.mjs"
export const console = config.console

KiyoCode uses the [AI SDK](https://ai-sdk.dev/) and [Models.dev](https://models.dev) to support **75+ LLM providers** and it supports running local models.

To add a provider you need to:

1. Add the API keys for the provider using the `/connect` command.
2. Configure the provider in your KiyoCode config.
...
You can customize the providers through the `provider` section in your KiyoCode
config.
...
KiyoCode Zen is a list of models provided by the KiyoCode team that have been
tested and verified to work well with KiyoCode. [Learn more](/docs/zen).

:::tip
If you are new, we recommend starting with KiyoCode Zen.
:::

1. Run the `/connect` command in the TUI, select `OpenCode Zen`, and head to [kiyocode.ai/auth](https://kiyocode.ai/zen).
...
It works like any other provider in KiyoCode and is completely optional to use.

---

## KiyoCode Go

KiyoCode Go is a low cost subscription plan that provides reliable access to popular open coding models provided by the KiyoCode team that have been
tested and verified to work well with KiyoCode.

1. Run the `/connect` command in the TUI, select `OpenCode Go`, and head to [kiyocode.ai/auth](https://kiyocode.ai/zen).
...
To use Amazon Bedrock with KiyoCode:
...
2. **Configure authentication** using one of the following methods:
   ...
   ***
   ...
   Set one of these environment variables while running kiyocode:
...
KiyoCode. Anthropic explicitly prohibits this.

Previous versions of KiyoCode came bundled with these plugins but that is no
longer the case as of 1.3.0

Other companies support freedom of choice with developer tooling - you can use
the following subscriptions in KiyoCode with zero setup:
...
You can configure kiyocode to use local models through [Atomic Chat](https://atomic.chat), a desktop application that runs local LLMs behind an OpenAI-compatible API server (default endpoint `http://127.0.0.1:1337/v1`).
...
2. Go to [Azure AI Foundry](https://ai.azure.com/) and deploy a model.
   ...
   :::note
   The deployment name must match the model name for kiyocode to work properly.
   :::
...
6. Run the `/models` command to select a model.
   ...
   You can also add models through your kiyocode config.
...
KiyoCode supports two authentication methods:

- **OAuth (Recommended)** — Sign in to your DigitalOcean account; KiyoCode uses your DigitalOcean API token directly for inference and discovers your Inference Routers.
...
3. Your browser opens to authorize KiyoCode. Sign in and approve.

   :::note
   KiyoCode requests `genai:read` and `inference:query` OAuth scopes. Your DigitalOcean API token is used directly for inference — no separate Model Access Key is created.
   :::
...
Inference Routers let you define a routing policy across multiple models — picking the cheapest, fastest, or most appropriate model per request based on the task. After OAuth, KiyoCode surfaces each router as `router:<router-name>` in the model picker.

Selecting a router model is a drop-in replacement for any other model — KiyoCode forwards your request and DigitalOcean picks the underlying model based on your router's policy. Learn more about [Inference Routers](https://docs.digitalocean.com/products/inference/how-to/use-inference-router/)
...
GitLab Duo support in KiyoCode is experimental. Features, configuration, and
behavior may change in future releases.
:::

KiyoCode integrates with the [GitLab Duo Agent Platform](https://docs.gitlab.com/user/duo_agent_platform/),
...
to store token in kiyocode auth storage.
:::

##### Self-Hosted GitLab

:::note[compliance note]
KiyoCode uses a small model for some AI tasks like generating the session title.
It is configured to use gpt-5-nano by default, hosted by Zen. To lock KiyoCode
...
When a `duo-workflow-*` model is selected, KiyoCode will:
...
4. Route tool execution requests through KiyoCode's permission-gated tool system
...
To use your GitHub Copilot subscription with kiyocode:
...
To use GMI Cloud with KiyoCode:
...
To use Google Vertex AI with KiyoCode:
...
2. Set the required environment variables:
   ...
   Set them while running kiyocode.
...
In the event you see a feature or model from Helicone that isn't configured automatically through kiyocode, you can always configure it yourself.
...
Helicone's [Sessions](https://docs.helicone.ai/features/sessions) feature lets you group related LLM requests together. Use the [opencode-helicone-session](https://github.com/H2Shami/opencode-helicone-session) plugin to automatically log each KiyoCode conversation as a session in Helicone.
...
The plugin injects `Helicone-Session-Id` and `Helicone-Session-Name` headers into your requests. In Helicone's Sessions page, you'll see each KiyoCode conversation listed as a separate session.
...
You can configure kiyocode to use local models through [llama.cpp's](https://github.com/ggml-org/llama.cpp) llama-server utility
...
You can configure kiyocode to use local models through LM Studio.
...
You can configure kiyocode to use local models through Ollama.

:::tip
Ollama can automatically configure itself for KiyoCode. See the [Ollama integration docs](https://docs.ollama.com/integrations/kiyocode) for details.
:::
...
To use Ollama Cloud with KiyoCode:
...
3. Copy the API key for use in KiyoCode.
...
6. **Important**: Before using cloud models in KiyoCode, you must pull the model information locally:
...
KiyoCode Zen is a list of tested and verified models provided by the KiyoCode team. [Learn more](/docs/zen).

1. Sign in to **<a href={console}>KiyoCode Zen</a>** and click **Create API Key**.

2. Run the `/connect` command and search for **KiyoCode Zen**.

   ```txt
   /connect
   ```

3. Enter your KiyoCode API key.
...
4. Many OpenRouter models are preloaded by default, run the `/models` command to select the one you want.
   ...
   You can also add additional models through your kiyocode config.
...
5. You can also customize them through your kiyocode config. Here's an example of specifying a provider
...
To use [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) with Kiyocode:
...
[SLAI](https://grape.skinnertopia.com) (Skinnertopia Lab for AI) exposes an OpenAI-compatible API for its own GRaPE and CRePE models, plus a selection of third-party models routed through Logfare AI.

1. Head over to the [SLAI Developer Portal](https://grape.skinnertopia.com/api/docs), create an account, and generate an API key. Keys start with `slai-`.

2. Run the `/connect` command, scroll down to **Other**, and enter a provider ID like `slai`.

   ```txt
   /connect
   ```

3. Paste your SLAI API key.

4. Create or update your `opencode.json`:
   ...
   ```json title="opencode.json"
   {
     ...
     "provider": {
       "slai": {
         ...
         "name": "SLAI",
         "options": {
           "baseURL": "https://grape.skinnertopia.com/api/openai/v1"
         },
         "models": {
   ...
   `GET /models` requires no authentication, so you can list the full, current set of model IDs (including any third-party Logfare-routed models) at any time:

   ```bash frame="none"
   curl https://grape.skinnertopia.com/api/openai/v1/models
   ```
...
GRaPE and CRePE models accept a `thinking_mode` field (`minimal | low | medium | high | xtra-hi`) that controls reasoning depth. CRePE models additionally accept `work_effort` (same values) to independently control agentic tool-use intensity. Set them per model — the field names must match the API exactly (snake_case), since `@ai-sdk/openai-compatible` passes model `options` through to the request body as-is:
  ...
  "provider": {
    "slai": {
      "models": {
        ...
        "crepe-2@bf16": {
          "options": { "thinking_mode": "high", "work_effort": "medium" }
        }
      }
    }
  }
...
:::note
Quota errors (`429 insufficient_quota`) require emailing contact@skinnertopia.com to request an increase.
:::
...
Don't have a Snowflake account? [Sign up for a free trial](https://signup.snowflake.com/?utm_source=kiyocode&utm_medium=docs&utm_campaign=cortex-provider).

kiyocode's core workflow for coding, editing files, and running commands relies on tool calling. Only the Claude and OpenAI families within Snowflake Cortex support this. The provider is limited to those families to support the core workflow.
...
5. Complete sign-in in the browser that opens. KiyoCode captures the OAuth callback automatically and stores the token — no copy/paste needed.
...
For CI or headless environments, set a PAT or JWT before starting kiyocode:
...
You can also customize models through your kiyocode config. Here's an example of specifying provider routing order.
...
2. Select **xAI Grok OAuth (SuperGrok Subscription)**. KiyoCode opens xAI's consent screen in your browser and waits for the callback on `http://127.0.0.1:56121/callback`.
...
KiyoCode refreshes the OAuth access token automatically. Any Grok or X Premium plan that includes Grok API access works; you do not need a separate `XAI_API_KEY`.

#### Option B — SuperGrok device-code (headless / remote server / VPS)

Use this when KiyoCode is running somewhere a browser can't reach the loopback redirect: a VPS, a remote dev box over SSH, inside Docker, in CI, etc. No callback port is opened on the host running KiyoCode — instead xAI hands the CLI a short code that you type into a browser on any other device (laptop, phone, …).
...
2. Select **xAI Grok OAuth (Headless / Remote / VPS)**. KiyoCode prints a verification URL and a short user code.

   ```txt
   Open https://x.ai/device on any device and enter code: ABCD-1234
   ```

3. Open the URL on a device that has a browser (your laptop or phone), enter the code, and approve the consent screen. KiyoCode polls xAI's token endpoint and stores the resulting OAuth tokens once you approve. Token refresh works the same as Option A.
...
:::tip
You can use any OpenAI-compatible provider with kiyocode. Most modern AI providers offer OpenAI-compatible APIs.
:::
...
The `limit` fields allow KiyoCode to understand how much context you have left. Standard providers pull these from models.dev automatically.
...
2. For custom providers, check the kiyocode config and:
   - Make sure the provider ID used in the `/connect` command matches the ID in your kiyocode config.
