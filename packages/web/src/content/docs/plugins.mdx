---
title: Plugins
description: Write your own plugins to extend KiyoCode.
---

Plugins allow you to extend KiyoCode by hooking into various events and customizing behavior. You can create plugins to add new features, integrate with external services, or modify KiyoCode's default behavior.
...
KiyoCode runs `bun install` at startup to install these. Your plugins and tools can then import them.
...
- `client`: An kiyocode SDK client for interacting with the AI.
...
Here are some examples of plugins you can use to extend kiyocode.
...
:::note
If you’re using the KiyoCode desktop app, it can send system notifications automatically when a response is ready or when a session errors.
:::
...
Prevent kiyocode from reading `.env` files:
...
Plugins can also add custom tools to kiyocode:
...
The `tool` helper creates a custom tool that kiyocode can call. It takes a Zod schema function and returns a tool definition with:

- `description`: What the tool does
- `args`: Zod schema for the tool's arguments
- `execute`: Function that runs when the tool is called

Your custom tools will be available to kiyocode alongside built-in tools.
...
Levels: `debug`, `info`, `warn`, `error`. See [SDK documentation](https://kiyocode.ai/docs/sdk) for details.
