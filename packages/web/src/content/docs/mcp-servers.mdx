...
You can add external tools to KiyoCode using the _Model Context Protocol_, or MCP. KiyoCode supports both local and remote servers.
...
You can define MCP servers in your [KiyoCode Config](https://kiyocode.ai/docs/config/) under `mcp`. Add each MCP with a unique name. You can refer to that MCP by name when prompting the LLM.
...
KiyoCode automatically handles OAuth authentication for remote MCP servers. When a server requires authentication, KiyoCode will:
...
If the server requires authentication, KiyoCode will prompt you to authenticate when you first try to use it. If not, you can [manually trigger the flow](#authenticating) with `opencode mcp auth <server-name>`.
...
The `mcp auth` command will open your browser for authorization. After you authorize, KiyoCode will store the tokens securely in `~/.local/share/opencode/mcp-auth.json`.
...
Your MCPs are available as tools in KiyoCode, alongside built-in tools. So you can manage them through the KiyoCode config like any other tool.
...
This will open a browser window to complete the OAuth flow and connect KiyoCode to your Sentry account.
