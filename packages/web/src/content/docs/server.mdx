---
title: Server
description: Interact with kiyocode server over HTTP.
---

import config from "../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

The `opencode serve` command runs a headless HTTP server that exposes an OpenAPI endpoint that an kiyocode client can use.
...
:::tip
Use the kiyocode server to interact with kiyocode programmatically.
:::

This architecture lets kiyocode support multiple clients and allows you to interact with kiyocode programmatically.

You can run `opencode serve` to start a standalone server. If you have the
kiyocode TUI running, `opencode serve` will start a new server.
...
The [`/tui`](#tui) endpoint can be used to drive the TUI through the server. For example, you can prefill or run a prompt. This setup is used by the KiyoCode [IDE](/docs/ide) plugins.
...
The kiyocode server exposes the following APIs.
