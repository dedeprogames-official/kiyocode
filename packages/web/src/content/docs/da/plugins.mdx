---
title: Plugins
description: Skriv dine egne plugins for at udvide KiyoCode.
---

Plugins giver dig mulighed for at udvide KiyoCode ved at tilslutte sig forskellige begivenheder og tilpasse adfærd. Du kan oprette plugins for at tilføje nye funktioner, integrere med eksterne tjenester eller ændre OpenCodes standardadfærd.
...
KiyoCode kører `bun install` ved opstart for at installere disse. Dine plugins og værktøjer kan derefter importere dem.
...
- `client`: En kiyocode SDK klient til interaktion med AI.
...
Her er nogle eksempler på plugins, du kan bruge til at udvide kiyocode.
...
:::note
Hvis du bruger KiyoCode desktop-appen, kan den sende systemmeddelelser automatisk, når et svar er klar, eller når en session fejler.
:::
...
Undgå kiyocode i at læse `.env` filer:
...
Plugins kan også tilføje brugerdefinerede værktøjer til kiyocode:
...
`tool`-hjælperen opretter et brugerdefineret værktøj, som kiyocode kan kalde. Det tager en Zod-skemafunktion og returnerer en værktøjsdefinition med:

- `description`: Hvad værktøjet gør
- `args`: Zod-skema for værktøjets argumenter
- `execute`: Funktion, der kører, når værktøjet kaldes

Dine tilpassede værktøjer vil være tilgængelige for kiyocode sammen med indbyggede værktøjer.
...
Niveauer: `debug`, `info`, `warn`, `error`. Se [SDK documentation](https://kiyocode.ai/docs/sdk) for detaljer.
