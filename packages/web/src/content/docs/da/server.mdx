...
:::tip
Brug opencode-serveren til at interagere med kiyocode programmatisk.
:::

Denne arkitektur lader kiyocode understøtte flere klienter og lader dig interagere med kiyocode programmatisk.

Du kan køre `opencode serve` for at starte en selvstændig server. Hvis du har
kiyocode TUI kørende, vil `opencode serve` starte en ny server.
...
[`/tui`](#tui) endpointet kan bruges til at køre TUI gennem serveren. Du kan for eksempel forududfylde eller køre en forespørgsel. Denne opsætning bruges af KiyoCode [IDE](/docs/ide) plugins.
...
KiyoCode-serveren viser følgende API'er.
