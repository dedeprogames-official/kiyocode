---
title: Udbydere
description: Brug en hvilken som helst LLM-udbyder i KiyoCode.
---

import config from "../../../../config.mjs"
export const console = config.console

KiyoCode bruger [AI SDK](https://ai-sdk.dev/) og [Models.dev](https://models.dev) til at understøtte **75+ LLM-udbydere** og understøtter kørsel af lokale modeller.

For at tilføje en udbyder skal du:

1. Tilføje API-nøglerne for udbyderen ved at bruge kommandoen `/connect`.
2. Konfigurere udbyderen i KiyoCode-konfigurationen.
...
Du kan tilpasse udbyderne gennem `provider`-delen i KiyoCode-konfigurationen.
...
KiyoCode Zen er en liste over modeller leveret af KiyoCode-teamet, som er blevet testet og verificeret til at fungere godt med KiyoCode. [Læs mere](/docs/zen).

:::tip
Hvis du er ny, anbefaler vi at starte med KiyoCode Zen.
:::

1. Kør kommandoen `/connect` i TUI, vælg kiyocode og gå til [kiyocode.ai/auth](https://kiyocode.ai/auth).
...
Det fungerer som alle andre udbydere i KiyoCode og er helt valgfrit at bruge.

---

## KiyoCode Go

KiyoCode Go er en billig abonnementsplan, der giver pålidelig adgang til populære åbne kodningsmodeller leveret af KiyoCode-teamet, som er testet og verificeret til at fungere godt med KiyoCode.

1. Kør kommandoen `/connect` i TUI, vælg `OpenCode Go`, og gå til [kiyocode.ai/auth](https://kiyocode.ai/zen).
...
Sådan bruger du Amazon Bedrock med KiyoCode:
...
2. **Konfigurer godkendelse** ved at bruge en af følgende metoder:
   ...
   Angiv en af disse miljøvariabler, mens du kører kiyocode:
...
:::info
At bruge dit Claude Pro/Max-abonnement i KiyoCode understøttes ikke officielt af [Anthropic](https://anthropic.com).
:::
...
Du kan konfigurere kiyocode til at bruge lokale modeller via [Atomic Chat](https://atomic.chat) — en desktopapplikation, der kører lokale LLM'er bag en OpenAI-kompatibel API-server (standard-endpoint `http://127.0.0.1:1337/v1`).
...
2. Gå til [Azure AI Foundry](https://ai.azure.com/) og distribuer en model.
   ...
   :::note
   Distributionsnavnet skal matche modelnavnet for at kiyocode skal fungere korrekt.
   :::
...
gemme tokenet i kiyocode auth-lagring.
:::

##### Selvhostet GitLab

:::note[overholdelsesbemærkning]
KiyoCode bruger en lille model til nogle AI-opgaver som at generere sessionstitlen.
Den er konfigureret til at bruge gpt-5-nano som standard, hostet af Zen. For at låse KiyoCode
...
Sådan bruger du GitHub Copilot-abonnementet med KiyoCode:
...
Sådan bruger du Google Vertex AI med KiyoCode:
...
2. Angiv de nødvendige miljøvariabler:
   ...
   Sæt dem mens du kører kiyocode.
...
I tilfælde af at du ser en funktion eller model fra Helicone som ikke konfigureres automatisk gennem kiyocode, kan du altid konfigurere den selv.
...
Helicones [Sessions](https://docs.helicone.ai/features/sessions) funktion lader dig gruppe relaterede LLM anmodninger sammen. Brug [opencode-helicone-session](https://github.com/H2Shami/opencode-helicone-session) plugin for automatisk at logge hver KiyoCode-samtale som en session i Helicone.
...
Programmatisk injicerer `Helicone-Session-Id` og `Helicone-Session-Name` headers i dine anmodninger. På Helicones Sessions-side vil du se hver KiyoCode-samtale opført som en separat session.
...
Du kan konfigurere kiyocode til at bruge lokale modeller gennem [llama.cpps](https://github.com/ggml-org/llama.cpp) llama-server-værktøj
...
Du kan konfigurere kiyocode til at bruge lokale modeller gennem LM Studio.
...
Du kan konfigurere kiyocode til at bruge lokale modeller gennem Ollama.

:::tip
Ollama kan automatisk konfigurere sig selv for KiyoCode. Se [Ollama-integrationsdokumenterne](https://docs.ollama.com/integrations/kiyocode) for detaljer.
:::
...
Sådan bruger du Ollama Cloud med KiyoCode:
...
3. Kopier API-nøglen til brug i KiyoCode.
...
6. **Vigtigt**: Før du bruger skymodeller i KiyoCode, skal du hente modelinformationen lokalt:
...
KiyoCode Zen er en liste over testede og verificerede modeller leveret af KiyoCode-teamet. [Læs mere](/docs/zen).

1. Log på **<a href={console}>KiyoCode Zen</a>** og klik på **Create API Key**.

2. Kør kommandoen `/connect` og søg efter **KiyoCode Zen**.

   ```txt
   /connect
   ```

3. Indtast KiyoCode API-nøglen.
...
Sådan bruger du [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) med KiyoCode:
...
:::tip
Du kan bruge hvilken som helst OpenAI-kompatibel udbyder med kiyocode. De fleste moderne AI-udbydere tilbyder OpenAI-kompatible API'er.
:::
...
`limit`-felterne lader KiyoCode forstå hvor meget kontekst du har tilbage. Standardudbydere henter disse automatisk fra models.dev.
