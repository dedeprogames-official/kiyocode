...
For at fejlfinde problemer med KiyoCode, start med at tjekke logfilerne og de lokale data, den gemmer på disken.
...
kiyocode gemmer sessionsdata og andre applikationsdata på disken på:
...
KiyoCode Desktop kører en lokal KiyoCode-server (`opencode-cli` sidevognen) i baggrunden. De fleste problemer er forårsaget af et plugin, der fungerer dårligt, en ødelagt cache eller en dårlig serverindstilling.
...
KiyoCode kan også indlæse lokale plugins fra disken. Flyt disse midlertidigt væk (eller giv mappen nyt navn) og start skrivebordsappen på ny:
...
Hvis deaktivering af plugins ikke hjælper (eller en plugin-installation sidder fast), tøm cachen så KiyoCode kan genopbygge den.

1. Afslut KiyoCode Desktop helt.
2. Slet cache-mappen:
...
3. Start KiyoCode Desktop på ny.

---

### Løs problemer med serverforbindelse

KiyoCode Desktop kan enten starte sin egen lokale server (standard) eller forbinde til en server URL, du har konfigureret.
...
På Windows kræver KiyoCode Desktop Microsoft Edge **WebView2 Runtime**. Hvis appen åbnes i et tomt vindue eller ikke starter, installer/opdater WebView2 og prøv igen.

---

### Windows: Generelle ydeevneproblemer

Hvis du oplever langsom ydeevne, problemer med filadgang eller terminalproblemer på Windows, kan du prøve at bruge [WSL (Windows Subsystem for Linux)](/docs/windows-wsl). WSL giver et Linux-miljø som fungerer mere sømløst med funktionerne i KiyoCode.

---

### Meddelelser vises ikke

KiyoCode Desktop viser kun systemvarsler når:

- varsler er aktiveret for KiyoCode i dine OS-indstillinger, og
...
1. Afslut KiyoCode Desktop.
2. Find og slet disse filer (de findes i KiyoCode Desktop-appens datamappe):
...
Hvis du oplever problemer med KiyoCode:
...
Hvis du støder på API-kaldsfejl, kan dette skyldes forældede udbyderpakker. kiyocode installerer dynamisk udbyderpakker (OpenAI, Anthropic, Google, etc.) efter behov og cacher dem lokalt.
...
2. Start kiyocode på ny for at installere de nyeste udbyderpakker på ny

Dette vil tvinge kiyocode til at downloade de nyeste versioner af udbyderpakkerne, som ofte løser kompatibilitetsproblemer med modelparametre og API-ændringer.
...
kiyocode vil opdage om du bruger Wayland og foretrække `wl-clipboard`, ellers vil den prøve at finde udklipsholderværktøjer i rækkefølgen: `xclip` og `xsel`.
