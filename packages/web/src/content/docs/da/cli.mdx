---
title: CLI
description: KiyoCode CLI muligheder og kommandoer.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

KiyoCode CLI starter som standard [TUI](/docs/tui), når den kører uden nogen argumenter.

```bash
opencode
```

Men den accepterer også kommandoer som dokumenteret på denne side. Dette giver dig mulighed for at interagere med KiyoCode programmatisk.
...
Start KiyoCode terminalbrugergrænsefladen.
...
Administrer agenter i KiyoCode.
...
Tilslut en terminal til en allerede kørende KiyoCode backend-server startet via `serve` eller `web` kommandoer.

```bash
opencode attach [url]
```

Dette gør det muligt at bruge TUI med en ekstern KiyoCode backend. For eksempel:
...
KiyoCode drives af udbyderlisten på [Models.dev](https://models.dev), så du kan bruge `opencode auth login` til at konfigurere API-nøgler for enhver udbyder, du vil bruge. Dette er gemt i `~/.local/share/opencode/auth.json`.

```bash
opencode auth login
```

Når KiyoCode starter op, indlæser den udbyderne fra legitimationsfilen. Og hvis der er nogen nøgler defineret i dine miljøer eller en `.env` fil i dit projekt.
...
Brug flaget `--refresh` til at opdatere den cachelagrede modelliste. Dette er nyttigt, når nye modeller er blevet tilføjet til en udbyder, og du vil se dem i KiyoCode.
...
Kør kiyocode i ikke-interaktiv tilstand ved at sende en prompt direkte.
...
Start en hovedløs KiyoCode-server til API-adgang. Tjek [server docs](/docs/server) for den fulde HTTP-grænseflade.
...
Administrator KiyoCode sessionsholder.
...
Liste over alle KiyoCode sessioner.
...
Vis tokenbrug og omkostningsstatistikker for dine KiyoCode-sessioner.
...
Importer sessionsdata fra en JSON fil eller KiyoCode del URL.
...
Start en hovedløs KiyoCode-server med en webgrænseflade.

```bash
opencode web
```

Dette starter en HTTP-server og åbner en webbrowser for at få adgang til KiyoCode via en webgrænseflade. Indstil `OPENCODE_SERVER_PASSWORD` for at aktivere HTTP grundlæggende godkendelse (brugernavn er standard til `opencode`).
...
Afinstaller KiyoCode og fjern alle relaterede filer.
...
Opdaterer KiyoCode til den nyeste version eller en specifik version.
...
kiyocode CLI tager følgende globale flag.
...
KiyoCode kan konfigureres ved hjælp af miljøvariabler.
