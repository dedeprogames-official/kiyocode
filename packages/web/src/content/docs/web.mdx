---
title: Web
description: Using KiyoCode in your browser.
---

KiyoCode can run as a web application in your browser, providing the same powerful AI coding experience without needing a terminal.

![KiyoCode Web - New Session](../../assets/web/web-homepage-new-session.png)
...
This starts a local server on `127.0.0.1` with a random available port and automatically opens KiyoCode in your default browser.
...
By default, KiyoCode picks an available port. You can specify a port:
...
By default, the server binds to `127.0.0.1` (localhost only). To make KiyoCode accessible on your network:

```bash
opencode web --hostname 0.0.0.0
```

When using `0.0.0.0`, KiyoCode will display both local and network addresses:
...
Once started, the web interface provides access to your KiyoCode sessions.
