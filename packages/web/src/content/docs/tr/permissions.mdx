...
kiyocode, belirli bir eylemin otomatik olarak mı çalıştırılacağına, size sorulacağına veya engelleneceğine karar vermek için `permission` yapılandırmasını kullanır.
...
kiyocode'un başlatıldığı çalışma dizini dışındaki yollara dokunan araç çağrılarına izin vermek için `external_directory` kullanın. Bu, girdi olarak bir yolu alan tüm araçlar için geçerlidir (örneğin `read`, `edit`, `glob`, `grep` ve birçok `bash` komutu).
...
kiyocode izinleri araç adına ve birkaç güvenlik önlemine göre anahtarlanır:
...
Hiçbir şey belirtmezseniz kiyocode izin verilen varsayılanlardan başlar:
...
kiyocode onay istediğinde kullanıcı arayüzü üç sonuç sunar:

- `once` — yalnızca bu isteği onayla
- `always` — önerilen kalıplarla eşleşen gelecekteki istekleri onaylayın (geçerli kiyocode oturumunun geri kalanı için)
