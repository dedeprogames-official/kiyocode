...
kiyocode ile ilgili bir sorunu ayiklamak icin once loglari ve diskte tuttugu yerel verileri kontrol edin.
...
kiyocode, oturum verilerini ve diger uygulama verilerini diskte su konumda saklar:
...
kiyocode Desktop arka planda yerel bir kiyocode sunucusu (`opencode-cli` sidecar) calistirir. Sorunlarin cogu bozuk bir plugin, hasarli cache veya hatali sunucu ayarindan kaynaklanir.
...
kiyocode diskten yerel plugin de yukleyebilir. Bu dizinleri gecici olarak tasiyin (veya klasoru yeniden adlandirin) ve uygulamayi yeniden baslatin:
...
Pluginleri kapatmak ise yaramazsa (veya plugin kurulumu takili kaldiysa), kiyocode'un cache'i yeniden olusturmasi icin cache'i temizleyin.

1. kiyocode Desktop'u tamamen kapatin
...
3. kiyocode Desktop'u yeniden baslatin

---

### Sunucu bağlantı sorunlarını düzeltin

kiyocode Desktop ya kendi yerel sunucusunu baslatir (varsayilan) ya da sizin tanimladiginiz bir sunucu URL'sine baglanir.
...
Windows'ta kiyocode Desktop, Microsoft Edge **WebView2 Runtime** gerektirir. Uygulama bos pencereyle aciliyorsa veya hic baslamiyorsa WebView2'yi kurup/guncelleyip tekrar deneyin.

---

### Windows: Genel performans sorunları

Windows'ta yavaslik, dosya erisim sorunlari veya terminal problemleri yasiyorsaniz [WSL (Windows Subsystem for Linux)](/docs/windows-wsl) kullanmayi deneyin. WSL, kiyocode ozellikleriyle daha sorunsuz calisan bir Linux ortami saglar.

---

### Bildirimler görünmüyor

kiyocode Desktop sistem bildirimlerini yalnizca su durumlarda gosterir:

- Isletim sistemi ayarlarinizda kiyocode icin bildirimler etkinse
...
1. kiyocode Desktop'u kapatin
2. Su dosyalari bulun ve silin (kiyocode Desktop uygulama veri dizininde yer alirlar):
...
kiyocode ile ilgili bir sorun yasiyorsaniz:
...
API cagrisi hatalari aliyorsaniz bunun nedeni guncel olmayan provider paketleri olabilir. kiyocode, provider paketlerini (OpenAI, Anthropic, Google vb.) gerektikce dinamik yukler ve yerelde onbellekler.
...
2. En guncel provider paketlerini yeniden kurmak icin kiyocode'u yeniden baslatin

Bu, kiyocode'un en yeni provider paket surumlerini indirmesini zorlar ve model parametresi/API degisikliklerinden kaynakli uyumsuzluklari cogu zaman cozer.
...
kiyocode, Wayland kullandiginizi algilarsa `wl-clipboard` tercihi yapar. Aksi halde sirayla `xclip` ve `xsel` araclarini arar.
