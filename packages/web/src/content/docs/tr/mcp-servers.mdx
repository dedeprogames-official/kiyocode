...
_Model Bağlam Protokolü_ veya MCP kullanarak kiyocode'a harici araçlar ekleyebilirsiniz. kiyocode hem yerel hem de uzak sunucuları destekler.
...
MCP sunucularını [kiyocode Config](https://kiyocode.ai/docs/config/)'nizde `mcp` altında tanımlayabilirsiniz. Her MCP'ü benzersiz bir adla ekleyin. LLM isteminde bulunurken bu MCP adına başvurabilirsiniz.
...
kiyocode, uzak MCP sunucuları için OAuth kimlik doğrulamasını otomatik olarak yönetir. Bir sunucu kimlik doğrulaması gerektirdiğinde kiyocode şunları yapar:
...
Sunucu kimlik doğrulama gerektiriyorsa kiyocode, onu ilk kez kullanmaya çalıştığınızda kimlik doğrulamanızı isteyecektir. Değilse, `opencode mcp auth <server-name>` ile [manually trigger the flow](#authenticating) yapabilirsiniz.
...
`mcp auth` komutu tarayıcınızı yetkilendirme için açacaktır. Yetkilendirmenizin ardından kiyocode, belirteçleri `~/.local/share/opencode/mcp-auth.json`'de güvenli bir şekilde saklar.
...
MCP'leriniz yerleşik araçların yanı sıra kiyocode'da araç olarak mevcuttur. Böylece bunları diğer araçlar gibi kiyocode yapılandırması aracılığıyla yönetebilirsiniz.
...
Bu, OAuth akışını tamamlamak ve kiyocode'u Sentry hesabınıza bağlamak için bir tarayıcı penceresi açacaktır.
