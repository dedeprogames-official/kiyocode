...
kiyocode'da iki tür agent vardır; birincil agent'lar ve alt agent'lar.
...
kiyocode, **Build** ve **Plan** olmak üzere iki yerleşik birincil agent ile birlikte gelir. Bunlara aşağıda bakacağız.
...
KiyoCode, **General**, **Explore** ve **Scout** olmak üzere üç yerleşik alt agent ile birlikte gelir. Buna aşağıda bakacağız.

---

## Yerleşik

KiyoCode iki yerleşik birincil agent ve üç yerleşik alt agent ile birlikte gelir.
...
Sıcaklık belirtilmezse kiyocode modeline özgü varsayılanları kullanır; çoğu model için genellikle 0, Qwen modelleri için 0,55.
...
Bu yol, yapılandırma dosyasının bulunduğu yere göredir. Yani bu hem global kiyocode yapılandırması hem de projeye özel yapılandırma için işe yarar.
...
kiyocode yapılandırmanızdaki model kimliği `provider/model-id` biçimini kullanır. Örneğin, [KiyoCode Zen](/docs/zen) kullanıyorsanız, GPT 5.1 Codex için `opencode/gpt-5.1-codex` kullanırsınız.
...
:::tip
Paylaşmak istediğiniz bir agent'ınız var mı? [Submit a PR](https://github.com/dedeprogames-official/kiyocode).
:::
