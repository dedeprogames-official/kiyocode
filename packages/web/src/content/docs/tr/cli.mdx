---
title: CLI
description: kiyocode CLI seçenekleri ve komutları.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

kiyocode CLI herhangi bir argüman olmadan çalıştırıldığında varsayılan olarak [TUI](/docs/tui)'yi başlatır.

```bash
opencode
```

Ancak bu sayfada belgelendiği gibi komutları da kabul eder. Bu, kiyocode ile programlı olarak etkileşim kurmanıza olanak tanır.
...
kiyocode terminal kullanıcı arayüzünü başlatır.
...
kiyocode CLI ayrıca aşağıdaki komutlara da sahiptir.

---

### agent

kiyocode için agent'ları yönetin.
...
`serve` veya `web` komutlarıyla başlatılan, halihazırda çalışan bir kiyocode arka uç sunucusuna bir terminal ekleyin.

```bash
opencode attach [url]
```

Bu, TUI öğesinin uzak bir kiyocode arka ucuyla kullanılmasına olanak tanır. Örneğin:
...
kiyocode, [Models.dev](https://models.dev) adresindeki sağlayıcı listesi tarafından desteklenmektedir, dolayısıyla kullanmak istediğiniz herhangi bir sağlayıcı için API anahtarlarını ayarlamak üzere `opencode auth login` kullanabilirsiniz. Bu `~/.local/share/opencode/auth.json`'de saklanıyor.

```bash
opencode auth login
```

kiyocode başlatıldığında sağlayıcıları kimlik bilgileri dosyasından yükler. Ayrıca ortam değişkenlerindeki veya projenizdeki `.env` dosyasındaki anahtarları da kullanır.
...
Önbelleğe alınan model listesini güncellemek için `--refresh` bayrağını kullanın. Bu, bir sağlayıcıya yeni modeller eklendiğinde ve bunları kiyocode'da görmek istediğinizde kullanışlıdır.
...
Doğrudan bir istem ileterek kiyocode'u etkileşimli olmayan modda çalıştırın.
...
| <nobr><code>{"--attach"}</code></nobr>   |      | Çalışan bir kiyocode sunucusuna ekleyin (ör. http://localhost:4096)                           |
| <nobr><code>{"--password"}</code></nobr> | `-p` | Temel kimlik doğrulama parolası (varsayılan: `OPENCODE_SERVER_PASSWORD`)                      |
| <nobr><code>{"--username"}</code></nobr> | `-u` | Temel kimlik doğrulama kullanıcı adı (varsayılan: `OPENCODE_SERVER_USERNAME` veya `opencode`) |
...
API erişimi için headless bir kiyocode sunucusu başlatır. Tam HTTP arayüzü için [server docs](/docs/server) sayfasına bakın.

```bash
opencode serve
```

Bu, TUI arayüzü olmadan kiyocode işlevselliğine API erişimi sağlayan bir HTTP sunucusunu başlatır. HTTP temel kimlik doğrulamasını etkinleştirmek için `OPENCODE_SERVER_PASSWORD` öğesini ayarlayın (kullanıcı adı varsayılan olarak `opencode` şeklindedir).
...
Tüm kiyocode oturumlarını listeleyin.
...
kiyocode oturumlarınız için belirteç kullanımı ve maliyet istatistiklerini gösterin.
...
Bir JSON dosyasından veya kiyocode paylaşımından URL oturum verilerini içe aktarın.

```bash
opencode import <file>
```

Yerel bir dosyadan veya kiyocode paylaşımından (URL) içe aktarabilirsiniz.
...
Web arayüzüyle başsız bir kiyocode sunucusu başlatın.

```bash
opencode web
```

Bu, bir HTTP sunucusunu başlatır ve bir web arayüzü aracılığıyla kiyocode'a erişmek için bir web tarayıcısı açar. HTTP temel kimlik doğrulamasını etkinleştirmek için `OPENCODE_SERVER_PASSWORD` öğesini ayarlayın (kullanıcı adı varsayılan olarak `opencode` şeklindedir).
...
kiyocode'u kaldırın ve ilgili tüm dosyaları kaldırın.
...
kiyocode'u en son sürüme veya belirli bir sürüme günceller.
...
kiyocode CLI aşağıdaki global bayrakları destekler.
...
kiyocode ortam değişkenleri kullanılarak yapılandırılabilir.
