---
title: TUI
description: KiyoCode terminal kullanıcı arayüzünü kullanma.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

KiyoCode, projelerinizde LLM ile çalışmak için etkileşimli bir terminal arayüzü (TUI) sunar.

KiyoCode'u çalıştırdığınızda mevcut dizin için TUI başlar.
...
KiyoCode TUI kullanırken `/` ve ardından komut adını yazarak hızlı eylemler çalıştırabilirsiniz. Örneğin:
...
KiyoCode'a bir sağlayıcı ekler. Mevcut sağlayıcıları seçip API anahtarları eklemenizi sağlar.
...
KiyoCode'dan çıkar. _Takma adlar_: `/quit`, `/q`
...
- `leader_timeout` - KiyoCode'un leader key sonrasında ne kadar bekleyeceğini kontrol eder. Varsayılan değer `2000`.
