---
title: Kurallar
description: kiyocode için özel talimatları ayarlayın.
---

Bir `AGENTS.md` dosyası oluşturarak kiyocode için özel talimatlar verebilirsiniz. Bu, Cursor rules yaklaşımına benzer. Özel projeniz için davranışını özelleştirmek amacıyla LLM'in içeriğine dahil edilecek talimatları içerir.

---

## Başlat

Yeni bir `AGENTS.md` dosyası oluşturmak için `/init` komutunu kiyocode'da çalıştırabilirsiniz.

:::tip
Projenizin `AGENTS.md` dosyasını Git'e göndermelisiniz.
:::

Bu, projenin neyle ilgili olduğunu anlamak için projenizi ve tüm içeriğini tarayacak ve onunla bir `AGENTS.md` dosyası oluşturacaktır. Bu, kiyocode'un projede daha iyi gezinmesine yardımcı olur.
...
kiyocode ayrıca `AGENTS.md` dosyasının birden fazla konumdan okunmasını da destekler. Ve bu farklı amaçlara hizmet ediyor.
...
Ayrıca `~/.config/opencode/AGENTS.md` dosyasında genel kurallara sahip olabilirsiniz. Bu, tüm kiyocode oturumlarına uygulanır.
...
kiyocode, Claude Code'dan geçiş yapan kullanıcılar için yedek olarak Claude Code'un dosya kurallarını destekler:
...
Kiyocode başlatıldığında kural dosyalarını şu sırayla arar:
...
Kiyocode, `AGENTS.md`'daki dosya referanslarını otomatik olarak ayrıştırmasa da, benzer işlevleri iki şekilde elde edebilirsiniz:
...
`AGENTS.md`'nizde açık talimatlar sağlayarak kiyocode'u harici dosyaları okumayı öğretebilirsiniz. İşte pratik bir örnek:
...
- kiyocode'un dosyaları yalnızca belirli bir görev için gerektiğinde yüklediğinden emin olun
