...
KiyoCode Go, popüler açık kodlama modellerine güvenilir erişim sağlayan düşük maliyetli bir aboneliktir — **ilk ayınız için 5$**, sonrasında **aylık 10$**.

Go, KiyoCode'daki diğer sağlayıcılar gibi çalışır. KiyoCode Go'ya abone olur ve API anahtarınızı alırsınız. Bu **tamamen isteğe bağlıdır** ve KiyoCode'u kullanmak için bunu kullanmanıza gerek yoktur.
...
KiyoCode Go, bu modellere **ilk ayınız için 5$**, ardından **aylık 10$** karşılığında erişmenizi sağlar.

---

## Nasıl çalışır?

KiyoCode Go, KiyoCode'daki diğer sağlayıcılar gibi çalışır.

1. **<a href={console}>KiyoCode Zen</a>**'e giriş yapın, Go'ya abone olun ve API anahtarınızı kopyalayın.
...
:::note
Her çalışma alanından yalnızca bir üye KiyoCode Go'ya abone olabilir.
:::
...
KiyoCode Go aşağıdaki limitleri içerir:
...
KiyoCode yapılandırmanızdaki [model id](/docs/config/#models) formatı `opencode-go/<model-id>` şeklindedir. Örneğin, Kimi K2.7 Code için yapılandırmanızda `opencode-go/kimi-k2.7-code` kullanmalısınız.
...
KiyoCode Go'yu şu amaçlarla oluşturduk:
...
4. KiyoCode ile diğer sağlayıcıları da kullanmanıza olanak tanıyarak **kilitlenme (vendor lock-in) olmamasını** sağlamak.
