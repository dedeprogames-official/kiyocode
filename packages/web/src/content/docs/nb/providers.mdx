---
title: Leverandører
description: Bruke en hvilken som helst LLM-leverandør i KiyoCode.
---

import config from "../../../../config.mjs"
export const console = config.console

KiyoCode bruker [AI SDK](https://ai-sdk.dev/) og [Models.dev](https://models.dev) for å støtte **75+ LLM leverandører** og den støtter kjøring av lokale modeller.

For å legge til en leverandør må du:

1. Legg til API-nøkkelen for leverandøren ved å bruke kommandoen `/connect`.
2. Konfigurer leverandøren i KiyoCode-konfigurasjonen.
...
Du kan tilpasse leverandørene gjennom `provider`-delen i KiyoCode
konfig.
...
KiyoCode Zen er en liste over modeller levert av KiyoCode-teamet som har vært
testet og verifisert for å fungere godt med KiyoCode. [Finn ut mer](/docs/zen).

:::tip
Hvis du er ny, anbefaler vi å starte med KiyoCode Zen.
:::

1. Kjør kommandoen `/connect` i TUI, velg `OpenCode Zen` og gå til [kiyocode.ai/auth](https://kiyocode.ai/zen).
...
Det fungerer som alle andre leverandører i KiyoCode og er helt valgfritt å bruke.

---

## KiyoCode Go

KiyoCode Go er en lavpris abonnementsplan som gir pålitelig tilgang til populære åpne kodemodeller levert av KiyoCode-teamet som har vært
testet og verifisert for å fungere godt med KiyoCode.

1. Kjør kommandoen `/connect` i TUI, velg `OpenCode Go`, og gå til [kiyocode.ai/auth](https://kiyocode.ai/zen).
...
Slik bruker du Amazon Bedrock med KiyoCode:
...
2. **Konfigurer autentisering** ved å bruke en av følgende metoder:
   ...
   Angi en av disse miljøvariablene mens du kjører KiyoCode:
...
:::info
Å bruke Claude Pro/Max-abonnementet ditt i kiyocode støttes ikke offisielt av [Anthropic](https://anthropic.com).
:::
...
Du kan konfigurere kiyocode til å bruke lokale modeller via [Atomic Chat](https://atomic.chat) — et skrivebordsprogram som kjører lokale LLM-er bak en OpenAI-kompatibel API-server (standard endepunkt `http://127.0.0.1:1337/v1`).
...
2. Gå til [Azure AI Foundry](https://ai.azure.com/) og distribuer en modell.
   ...
   :::note
   Distribusjonsnavnet må samsvare med modellnavnet for at KiyoCode skal fungere skikkelig.
   :::
...
5. Kjør kommandoen `/models` for å velge en modell.
   ...
   Du kan også legge til modeller gjennom KiyoCode-konfigurasjonen.
...
for å lagre token i KiyoCode auth-lagring.
:::

##### Selvhostet GitLab

:::note[Merknad om samsvar]
KiyoCode bruker en liten modell for noen AI oppgaver som å generere økttittelen.
Den er konfigurert til å bruke gpt-5-nano som standard, hostet av Zen. For å låse KiyoCode
...
Slik bruker du GitHub Copilot-abonnementet med KiyoCode:
...
Slik bruker du Google Vertex AI med KiyoCode:
...
2. Angi de nødvendige miljøvariablene:
   ...
   Sett dem mens du kjører KiyoCode.
...
I tilfelle du ser en funksjon eller modell fra Helicone som ikke konfigureres automatisk gjennom KiyoCode, kan du alltid konfigurere den selv.
...
Helicones [Sessions](https://docs.helicone.ai/features/sessions) funksjon lar deg gruppere relaterte LLM forespørsler sammen. Bruk [opencode-helicone-session](https://github.com/H2Shami/opencode-helicone-session) plugin for automatisk å logge hver KiyoCode-samtale som en økt i Helicone.
...
Programtillegget injiserer `Helicone-Session-Id` og `Helicone-Session-Name` headere i forespørslene dine. På Helicones Sessions-side vil du se hver KiyoCode-samtale oppført som en separat økt.
...
Du kan konfigurere KiyoCode for å bruke lokale modeller gjennom [llama.cpp](https://github.com/ggml-org/llama.cpp) llama-server-verktøyet.
...
Du kan konfigurere KiyoCode for å bruke lokale modeller gjennom LM Studio.
...
Du kan konfigurere KiyoCode for å bruke lokale modeller gjennom Ollama.

:::tip
Ollama kan automatisk konfigurere seg selv for KiyoCode. Se [Ollama-integrasjonsdokumentene](https://docs.ollama.com/integrations/kiyocode) for detaljer.
:::
...
Slik bruker du Ollama Cloud med KiyoCode:
...
3. Kopier API-nøkkelen for bruk i KiyoCode.
...
6. **Viktig**: Før du bruker skymodeller i KiyoCode, må du hente modellinformasjonen lokalt:
...
KiyoCode Zen er en liste over testede og verifiserte modeller levert av KiyoCode-teamet. [Finn ut mer](/docs/zen).

1. Logg på **<a href={console}>KiyoCode Zen</a>** og klikk på **Create API Key**.

2. Kjør kommandoen `/connect` og søk etter **KiyoCode Zen**.

   ```txt
   /connect
   ```

3. Skriv inn KiyoCode API-nøkkelen.
...
4. Mange OpenRouter-modeller er forhåndslastet som standard, kjør kommandoen `/models` for å velge den du ønsker.
   ...
   Du kan også legge til flere modeller gjennom KiyoCode-konfigurasjonen.
...
5. Du kan også tilpasse dem gjennom KiyoCode-konfigurasjonen. Her er et eksempel på å spesifisere en leverandør
...
Slik bruker du [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) med KiyoCode:
...
Du kan også tilpasse modeller gjennom KiyoCode-konfigurasjonen. Her er et eksempel på spesifisering av leverandørrutingsrekkefølge.
...
:::tip
Du kan bruke hvilken som helst OpenAI-kompatibel leverandør med KiyoCode. De fleste moderne AI-leverandører tilbyr OpenAI-kompatible APIer.
:::
...
`limit`-feltene lar KiyoCode forstå hvor mye kontekst du har igjen. Standardleverandører henter disse automatisk fra models.dev.
...
2. For tilpassede leverandører, sjekk KiyoCode-konfigurasjonen og:
   - Sørg for at leverandør-ID-en som brukes i `/connect`-kommandoen samsvarer med ID i KiyoCode-konfigurasjonen.
