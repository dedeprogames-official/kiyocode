...
De fleste populære leverandørene er forhåndslastet som standard. Hvis du har lagt til legitimasjonen for en leverandør gjennom kommandoen `/connect`, vil den være tilgjengelig når du starter KiyoCode.
...
Her er flere modeller som fungerer godt med KiyoCode, uten spesiell rekkefølge. (Dette er ikke en uttømmende liste og er heller ikke nødvendigvis oppdatert):
...
Her er hele ID `provider_id/model_id`. For eksempel, hvis du bruker [KiyoCode Zen](/docs/zen), vil du bruke `opencode/gpt-5.1-codex` for GPT 5.1 Codex.
...
Mange modeller støtter flere varianter med forskjellige konfigurasjoner. kiyocode leveres med innebygde standardvarianter for populære leverandører.

### Innebygde varianter

KiyoCode leveres med standardvarianter for mange leverandører:
...
Når KiyoCode starter opp, ser den etter modeller i følgende prioriterte rekkefølge:

1. Kommandolinjeflagget `--model` eller `-m`. Formatet er det samme som i konfigurasjonsfilen: `provider_id/model_id`.

2. Modelllisten i KiyoCode-konfigurasjonen.
