...
Du kan legge til eksterne verktøy i KiyoCode ved å bruke _Model Context Protocol_ eller MCP. KiyoCode støtter både lokale og eksterne servere.
...
Du kan definere MCP-servere i [KiyoCode Config](https://kiyocode.ai/docs/config/) under `mcp`. Legg til hver MCP med et unikt navn. Du kan referere til den MCP ved navn når du spør LLM-en.
...
KiyoCode håndterer automatisk OAuth-autentisering for eksterne MCP-servere. Når en server krever autentisering, vil KiyoCode:
...
Hvis serveren krever autentisering, vil KiyoCode be deg om å autentisere når du prøver å bruke den for første gang. Hvis ikke, kan du [utløse flyten manuelt](#authenticating) med `opencode mcp auth <server-name>`.
...
Kommandoen `mcp auth` åpner nettleseren din for autorisasjon. Etter at du har godkjent, vil KiyoCode lagre tokens sikkert i `~/.local/share/opencode/mcp-auth.json`.
...
MCP-ene dine er tilgjengelige som verktøy i KiyoCode, sammen med innebygde verktøy. Så du kan administrere dem gjennom KiyoCode-konfigurasjonen som alle andre verktøy.
...
Dette åpner et nettleservindu for å fullføre OAuth-flyten og koble KiyoCode til Sentry-kontoen din.
...
Programtillegget injiserer `Helicone-Session-Id` og `Helicone-Session-Name` overskrifter i forespørslene dine. På Helicones Sessions-side vil du se hver KiyoCode-samtale oppført som en separat økt.
