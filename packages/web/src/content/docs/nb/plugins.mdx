...
description: Skriv dine egne programtillegg for å utvide KiyoCode.
---

Plugins lar deg utvide KiyoCode ved å koble til ulike hendelser og tilpasse atferd. Du kan lage plugins for å legge til nye funksjoner, integrere med eksterne tjenester eller endre standardoppførselen til KiyoCode.
...
KiyoCode kjører `bun install` ved oppstart for å installere disse. Programtilleggene og verktøyene dine kan deretter importere dem.
...
- `client`: En KiyoCode SDK klient for samhandling med AI.
...
Her er noen eksempler på plugins du kan bruke for å utvide KiyoCode.
...
:::note
Hvis du bruker KiyoCode-skrivebordsappen, kan den sende systemvarsler automatisk når et svar er klart eller når en økt feiler.
:::
...
Hindre KiyoCode fra å lese `.env` filer:
...
Plugins kan også legge til egendefinerte verktøy til KiyoCode:
...
`tool`-hjelperen lager et tilpasset verktøy som KiyoCode kan kalle. Den tar en Zod-skjemafunksjon og returnerer en verktøydefinisjon med:

- `description`: Hva verktøyet gjør
- `args`: Zod-skjema for verktøyets argumenter
  - `execute`: Funksjon som kjører når verktøyet kalles

Dine egendefinerte verktøy vil være tilgjengelige for KiyoCode sammen med innebygde verktøy.
...
Nivåer: `debug`, `info`, `warn`, `error`. Se [SDK dokumentasjon](https://kiyocode.ai/docs/sdk) for detaljer.
