---
title: Konfigurasjon
description: Ved å bruke kiyocode JSON-konfigurasjonen.
---

Du kan konfigurere KiyoCode ved å bruke en JSON konfigurasjonsfil.

---

## Format

KiyoCode støtter både **JSON** og **JSONC** (JSON med kommentarer) formater.
...
Plasser din globale KiyoCode-konfigurasjon i `~/.config/opencode/opencode.json`. Bruk global konfigurasjon for brukerspesifikke preferanser som temaer, leverandører eller nøkkelbindinger.
...
Når KiyoCode starter opp, ser den etter en konfigurasjonsfil i gjeldende katalog eller går opp til nærmeste Git-katalog.
...
Konfigurasjonsfilen har et skjema som er definert i [**`kiyocode.ai/config.json`**](https://kiyocode.ai/config.json).
...
- `mdns` - Aktiver mDNS-tjenesteoppdagelse. Dette lar andre enheter på nettverket oppdage KiyoCode-serveren din.
...
Du kan konfigurere leverandørene og modellene du vil bruke i KiyoCode-konfigurasjonen gjennom alternativene `provider`, `model` og `small_model`.
...
Alternativet `small_model` konfigurerer en egen modell for lette oppgaver som tittelgenerering. Som standard prøver KiyoCode å bruke en rimeligere modell hvis en er tilgjengelig fra leverandøren din, ellers faller den tilbake til hovedmodellen din.
...
Standardagenten må være en primæragent (ikke en underagent). Dette kan være en innebygd agent som `"build"` eller `"plan"`, eller en [egendefinert agent](/docs/agents) du har definert. Hvis den angitte agenten ikke eksisterer eller er en subagent, vil KiyoCode falle tilbake til `"build"` med en advarsel.
...
KiyoCode vil automatisk laste ned eventuelle nye oppdateringer når den starter opp. Du kan deaktivere dette med alternativet `autoupdate`.
...
Som standard **tillater KiyoCode alle operasjoner** uten å kreve eksplisitt godkjenning. Du kan endre dette ved å bruke alternativet `permission`.
...
[Plugins](/docs/plugins) utvider KiyoCode med tilpassede verktøy, kroker og integrasjoner.
...
Dette er nyttig når du vil begrense KiyoCode til kun å bruke spesifikke leverandører i stedet for å deaktivere dem én etter én.
