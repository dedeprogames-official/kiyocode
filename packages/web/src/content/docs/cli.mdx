---
title: CLI
description: KiyoCode CLI options and commands.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

The KiyoCode CLI by default starts the [TUI](/docs/tui) when run without any arguments.

```bash
opencode
```

But it also accepts commands as documented on this page. This allows you to interact with KiyoCode programmatically.
...
Start the KiyoCode terminal user interface.
...
The KiyoCode CLI also has the following commands.

---

### agent

Manage agents for KiyoCode.
...
Attach a terminal to an already running KiyoCode backend server started via `serve` or `web` commands.

```bash
opencode attach [url]
```

This allows using the TUI with a remote KiyoCode backend. For example:
...
KiyoCode is powered by the provider list at [Models.dev](https://models.dev), so you can use `opencode auth login` to configure API keys for any provider you'd like to use. This is stored in `~/.local/share/opencode/auth.json`.

```bash
opencode auth login
```

When KiyoCode starts up it loads the providers from the credentials file. And if there are any keys defined in your environments or a `.env` file in your project.
...
Use the `--refresh` flag to update the cached model list. This is useful when new models have been added to a provider and you want to see them in KiyoCode.
...
Run kiyocode in non-interactive mode by passing a prompt directly.
...
| <nobr><code>{"--attach"}</code></nobr>   |       | Attach to a running kiyocode server (e.g., http://localhost:4096)          |
| <nobr><code>{"--password"}</code></nobr> | `-p`  | Basic auth password (defaults to `OPENCODE_SERVER_PASSWORD`)               |
| <nobr><code>{"--username"}</code></nobr> | `-u`  | Basic auth username (defaults to `OPENCODE_SERVER_USERNAME` or `opencode`) |
...
Start a headless KiyoCode server for API access. Check out the [server docs](/docs/server) for the full HTTP interface.

```bash
opencode serve
```

This starts an HTTP server that provides API access to kiyocode functionality without the TUI interface. Set `OPENCODE_SERVER_PASSWORD` to enable HTTP basic auth (username defaults to `opencode`).
...
Show token usage and cost statistics for your KiyoCode sessions.
...
Import session data from a JSON file or KiyoCode share URL.

```bash
opencode import <file>
```

You can import from a local file or an KiyoCode share URL.
...
Start a headless KiyoCode server with a web interface.

```bash
opencode web
```

This starts an HTTP server and opens a web browser to access KiyoCode through a web interface. Set `OPENCODE_SERVER_PASSWORD` to enable HTTP basic auth (username defaults to `opencode`).
...
Fetch and checkout a GitHub PR branch, then run KiyoCode.
...
Uninstall KiyoCode and remove all related files.
...
Updates kiyocode to the latest version or a specific version.
...
The kiyocode CLI takes the following global flags.
...
KiyoCode can be configured using environment variables.
