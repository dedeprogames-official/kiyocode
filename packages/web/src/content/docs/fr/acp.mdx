---
title: Support ACP
description: Utilisez KiyoCode dans n’importe quel éditeur compatible ACP.
---

KiyoCode prend en charge le [Agent Client Protocol](https://agentclientprotocol.com) (ACP), vous permettant de l'utiliser directement dans les éditeurs et IDE compatibles.
...
Pour utiliser KiyoCode via ACP, configurez votre éditeur pour exécuter la commande `opencode acp`.

La commande démarre KiyoCode en tant que sous-processus compatible ACP qui communique avec votre éditeur via JSON-RPC sur stdio.
...
Pour l'ouvrir, utilisez le nouvel agent « KiyoCode » dans le sélecteur d'agent AI Chat.
...
Pour utiliser KiyoCode comme agent ACP dans [CodeCompanion.nvim](https://github.com/olimorris/codecompanion.nvim), ajoutez ce qui suit à votre configuration Neovim :
...
Cette configuration configure CodeCompanion pour utiliser KiyoCode comme agent ACP pour le chat.
...
KiyoCode fonctionne de la même manière via ACP que dans le terminal. Toutes les fonctionnalités sont prises en charge :
...
- Serveurs MCP configurés dans votre configuration KiyoCode
