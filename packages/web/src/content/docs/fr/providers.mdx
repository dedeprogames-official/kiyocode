---
title: Fournisseurs
description: Utiliser n’importe quel fournisseur LLM en KiyoCode.
---

import config from "../../../../config.mjs"
export const console = config.console

KiyoCode utilise [AI SDK](https://ai-sdk.dev/) et [Models.dev](https://models.dev) pour prendre en charge **75+ fournisseurs LLM** et prend en charge l'exécution de modèles locaux.

Pour ajouter un fournisseur, vous devez :

1. Ajoutez les clés API pour le fournisseur à l'aide de la commande `/connect`.
2. Configurez le fournisseur dans votre configuration KiyoCode.
...
Vous pouvez personnaliser les fournisseurs via la section `provider` de votre KiyoCode
configuration.
...
KiyoCode Zen est une liste de modèles fournis par l'équipe KiyoCode qui ont été
testé et vérifié pour fonctionner correctement avec KiyoCode. [En savoir plus](/docs/zen).

:::tip
Si vous êtes nouveau, nous vous recommandons de commencer par KiyoCode Zen.
:::

1. Exécutez la commande `/connect` dans le TUI, sélectionnez kiyocode et dirigez-vous vers [kiyocode.ai/auth](https://kiyocode.ai/auth).
...
Il fonctionne comme n’importe quel autre fournisseur dans KiyoCode et son utilisation est totalement facultative.

---

## KiyoCode Go

KiyoCode Go est un plan d'abonnement à faible coût qui offre un accès fiable aux modèles de codage ouverts populaires fournis par l'équipe KiyoCode qui ont été
testé et vérifié pour fonctionner correctement avec KiyoCode.

1. Exécutez la commande `/connect` dans le TUI, sélectionnez `OpenCode Go` et rendez-vous sur [kiyocode.ai/auth](https://kiyocode.ai/zen).
...
Pour utiliser Amazon Bedrock avec KiyoCode :
...
Définissez l'une de ces variables d'environnement lors de l'exécution de kiyocode :
...
:::info
L'utilisation de votre abonnement Claude Pro/Max dans KiyoCode n'est pas officiellement prise en charge par [Anthropic](https://anthropic.com).
:::
...
Vous pouvez configurer kiyocode pour utiliser des modèles locaux via [Atomic Chat](https://atomic.chat), une application de bureau qui exécute des LLM locaux derrière un serveur API compatible OpenAI (point de terminaison par défaut `http://127.0.0.1:1337/v1`).
...
2. Accédez à [Azure AI Foundry](https://ai.azure.com/) et déployez un modèle.
   ...
   :::note
   Le nom du déploiement doit correspondre au nom du modèle pour que kiyocode fonctionne correctement.
   :::
...
Vous pouvez également ajouter des modèles via votre configuration kiyocode.
...
```bash
   GOOGLE_APPLICATION_CREDENTIALS=/path/to/service-account.json GOOGLE_CLOUD_PROJECT=your-project-id kiyocode
```
...
```jsonc title="~/.config/kiyocode/opencode.jsonc"
{
  "$schema": "https://kiyocode.ai/config.json",
  "provider": {
    "helicone": {
...
```bash
   AICORE_SERVICE_KEY='{"clientid":"...","clientsecret":"...","url":"...","serviceurls":{"AI_API_URL":"..."}}' kiyocode
```
...
4. Définissez éventuellement l'ID de déploiement et le groupe de ressources :
   ...
   ```bash
   AICORE_DEPLOYMENT_ID=your-deployment-id AICORE_RESOURCE_GROUP=your-resource-group kiyocode
   ```
...
Les champs `limit` permettent à KiyoCode de comprendre la quantité de contexte qu'il vous reste. Les fournisseurs standard les extraient automatiquement de models.dev.
...
2. Pour les fournisseurs personnalisés, vérifiez la configuration kiyocode et :
   - Assurez-vous que l'ID du fournisseur utilisé dans la commande `/connect` correspond à l'ID de votre configuration kiyocode.
