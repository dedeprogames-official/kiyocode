---
title: Plugins
description: Écrivez vos propres plugins pour étendre KiyoCode.
---

Les plugins vous permettent d'étendre KiyoCode en vous connectant à divers événements et en personnalisant le comportement. Vous pouvez créer des plugins pour ajouter de nouvelles fonctionnalités, intégrer des services externes ou modifier le comportement par défaut de KiyoCode.
...
KiyoCode exécute `bun install` au démarrage pour les installer. Vos plugins et outils peuvent ensuite les importer.
...
- `client` : un client SDK kiyocode pour interagir avec l'IA.
...
Voici quelques exemples de plugins que vous pouvez utiliser pour étendre kiyocode.
...
:::note
Si vous utilisez l'application de bureau KiyoCode, elle peut envoyer automatiquement des notifications système lorsqu'une réponse est prête ou en cas d'erreur de session.
:::
...
Empêchez kiyocode de lire les fichiers `.env` :
...
Les plugins peuvent également ajouter des outils personnalisés à kiyocode :
...
L'assistant `tool` crée un outil personnalisé que kiyocode peut appeler. Il prend une fonction de schéma Zod et renvoie une définition d'outil avec :
...
Vos outils personnalisés seront disponibles pour kiyocode aux côtés des outils intégrés.
...
Niveaux : `debug`, `info`, `warn`, `error`. Voir la [documentation du SDK](https://kiyocode.ai/docs/sdk) pour plus de détails.
