...
Možete dodati vanjske alate u KiyoCode koristeći _Model Context Protocol_, ili MCP. KiyoCode podržava i lokalne i udaljene servere.
...
Možete definirati MCP servere u vašoj [KiyoCode Config](https://kiyocode.ai/docs/config/) pod `mcp`. Dodajte svaki MCP sa jedinstvenim imenom. Možete se pozvati na taj MCP po imenu kada tražite LLM.
...
KiyoCode automatski rukuje OAuth autentifikacijom za udaljene MCP servere. Kada server zahtijeva autentifikaciju, KiyoCode će:
...
Ako server zahtijeva autentifikaciju, KiyoCode će vas tražiti prijavu pri prvom korištenju. Ako se to ne desi, možete [ručno pokrenuti tok](#authenticating) naredbom `opencode mcp auth <server-name>`.
...
Komanda `mcp auth` otvara pretraživač za autorizaciju. Nakon odobrenja, KiyoCode sigurno čuva tokene u `~/.local/share/opencode/mcp-auth.json`.
...
Vaši MCP serveri su dostupni kao alati u KiyoCode, zajedno s ugrađenim alatima. Možete njima upravljati kroz KiyoCode konfiguraciju kao i bilo kojim drugim alatom.
...
Ovo će otvoriti prozor pretraživača da završite OAuth tok i povežete KiyoCode sa vašim Sentry nalogom.
