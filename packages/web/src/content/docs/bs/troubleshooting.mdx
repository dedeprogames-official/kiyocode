...
Da biste otklonili probleme s KiyoCode, počnite provjeravanjem dnevnika i lokalnih podataka koje pohranjuje na disku.
...
KiyoCode pohranjuje podatke o sesiji i druge podatke aplikacije na disku na:
...
KiyoCode Desktop pokreće lokalni KiyoCode server (`opencode-cli` sidecar) u pozadini. Većina problema je uzrokovana nedostatkom dodatka, oštećenom keš memorijom ili lošim postavkama servera.
...
KiyoCode također može učitati lokalne dodatke s diska. Privremeno ih maknite s puta (ili preimenujte folder) i ponovo pokrenite desktop aplikaciju:
...
Ako onemogućavanje dodataka ne pomogne (ili se instalacija dodatka zaglavila), obrišite keš memoriju kako bi ga KiyoCode mogao ponovo izgraditi.

1. Potpuno zatvorite KiyoCode Desktop.
...
3. Ponovo pokrenite KiyoCode Desktop.

---

### Rješavanje problema sa vezom na serveru

KiyoCode Desktop može ili pokrenuti svoj lokalni server (podrazumevano) ili se povezati na URL servera koji ste konfigurisali.
...
Na Windows-u, KiyoCode Desktop zahtijeva Microsoft Edge **WebView2 Runtime**. Ako se aplikacija otvori u praznom prozoru ili se ne pokrene, instalirajte/ažurirajte WebView2 i pokušajte ponovo.

---

### Windows: Opšti problemi sa performansama

Ako imate spore performanse, probleme s pristupom datotekama ili probleme s terminalom na Windows-u, pokušajte koristiti [WSL (Windows podsistem za Linux)](/docs/windows-wsl). WSL pruža Linux okruženje koje radi neprimetnije sa KiyoCode karakteristikama.

---

### Obavještenja se ne prikazuju

KiyoCode Desktop prikazuje sistemska obavještenja samo kada:

- obavještenja su omogućena za KiyoCode u postavkama vašeg OS-a, i
...
1. Zatvorite KiyoCode Desktop.
2. Pronađite i izbrišite ove datoteke (oni žive u direktoriju podataka KiyoCode Desktop aplikacije):
...
Ako naiđete na greške API poziva, to može biti zbog zastarjelih paketa dobavljača. KiyoCode dinamički instalira pakete dobavljača (OpenAI, Anthropic, Google, itd.) po potrebi i kešira ih lokalno.
...
2. Ponovo pokrenite KiyoCode da ponovo instalirate najnovije pakete dobavljača

Ovo će prisiliti KiyoCode da preuzme najnovije verzije paketa dobavljača, što često rješava probleme kompatibilnosti s parametrima modela i promjenama API-ja.
...
KiyoCode će otkriti da li koristite Wayland i preferirate `wl-clipboard`, u suprotnom će pokušati pronaći alate međuspremnika po redoslijedu: `xclip` i `xsel`.
