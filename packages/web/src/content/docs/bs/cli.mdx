---
title: CLI
description: KiyoCode CLI opcije i naredbe.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

KiyoCode CLI po defaultu pokreće [TUI](/docs/tui) kada se pokrene bez ikakvih argumenata.

```bash
opencode
```

Ali takođe prihvata naredbe kao što je dokumentovano na ovoj stranici. Ovo vam omogućava programsku interakciju sa KiyoCode.
...
Pokrenite KiyoCode terminalski korisnički interfejs.
...
KiyoCode CLI takođe ima sljedeće naredbe.

---

### agent

Upravljajte KiyoCode agentima.
...
Priključite terminal na već pokrenut KiyoCode backend server pokrenut putem `serve` ili `web` naredbi.

```bash
opencode attach [url]
```

Ovo omogućava korištenje TUI-ja sa udaljenim KiyoCode backend-om. Na primjer:
...
KiyoCode pokreće lista provajdera na [Models.dev](https://models.dev), tako da možete koristiti `opencode auth login` da konfigurirate API ključeve za bilo kojeg provajdera kojeg želite koristiti. Ovo je pohranjeno u `~/.local/share/opencode/auth.json`.

```bash
opencode auth login
```

Kada se KiyoCode pokrene, učitava dobavljače iz datoteke vjerodajnica. I ako postoje neki ključevi definirani u vašim okruženjima ili `.env` fajl u vašem projektu.
...
Koristite `--refresh` zastavicu da ažurirate keširanu listu modela. Ovo je korisno kada su novi modeli dodani provajderu i želite da ih vidite u KiyoCode.
...
Pokrenite KiyoCode u neinteraktivnom modu tako što ćete direktno proslijediti prompt.
...
| <nobr><code>{"--attach"}</code></nobr>   |        | Priključite na pokrenuti KiyoCode server (npr. http://localhost:4096)                         |
| <nobr><code>{"--password"}</code></nobr> | `-p`   | Lozinka za osnovnu autentifikaciju (zadano: `OPENCODE_SERVER_PASSWORD`)                       |
| <nobr><code>{"--username"}</code></nobr> | `-u`   | Korisničko ime za osnovnu autentifikaciju (zadano: `OPENCODE_SERVER_USERNAME` ili `opencode`) |
...
Pokrenite KiyoCode headless server za API pristup. Pogledajte [server docs](/docs/server) za kompletan HTTP interfejs.

```bash
opencode serve
```

Ovo pokreće HTTP server koji pruža API pristup funkcionalnosti KiyoCode-a bez TUI interfejsa. Postavite `OPENCODE_SERVER_PASSWORD` da omogućite HTTP osnovnu auth (korisničko ime je zadano na `opencode`).
...
Prikaži statistiku upotrebe tokena i troškova za vaše KiyoCode sesije.
...
Uvezite podatke sesije iz JSON datoteke ili KiyoCode dijeljenog URL-a.

```bash
opencode import <file>
```

Možete uvesti iz lokalne datoteke ili KiyoCode dijeljenog URL-a.
...
Pokrenite KiyoCode headless server sa web interfejsom.

```bash
opencode web
```

Ovo pokreće HTTP server i otvara web pretraživač za pristup KiyoCode-u preko web interfejsa. Postavite `OPENCODE_SERVER_PASSWORD` da omogućite HTTP osnovnu auth (korisničko ime je zadano na `opencode`).
...
Deinstalirajte KiyoCode i uklonite sve povezane datoteke.
...
Ažurira KiyoCode na najnoviju verziju ili određenu verziju.
...
KiyoCode CLI prihvata sljedeće globalne zastavice.
...
KiyoCode se može konfigurirati pomoću varijabli okruženja.
