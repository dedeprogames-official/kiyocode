---
title: Dodaci
description: Napišite vlastite dodatke za proširenje KiyoCode.
---

Dodaci vam omogućavaju da proširite KiyoCode spajanjem na različite događaje i prilagođavanjem ponašanja. Možete kreirati dodatke za dodavanje novih funkcija, integraciju sa eksternim uslugama ili izmenu zadanog ponašanja KiyoCode.
...
KiyoCode pokreće `bun install` pri pokretanju da ih instalira. Vaši dodaci i alati ih zatim mogu uvesti.
...
- `client`: Kiyocode SDK klijent za interakciju sa AI.
...
Evo nekoliko primjera dodataka koje možete koristiti za proširenje KiyoCode.
...
Spriječite kiyocode da čita `.env` fajlove:
...
Dodaci također mogu dodati prilagođene alate u kiyocode:
...
Pomoćnik `tool` kreira prilagođeni alat koji kiyocode može pozvati. Uzima funkciju Zod sheme i vraća definiciju alata sa:

- `description`: Šta alat radi
- `args`: Zod šema za argumente alata
- `execute`: Funkcija koja se pokreće kada se pozove alat
  Vaši prilagođeni alati će biti dostupni za kiyocode zajedno sa ugrađenim alatima.
...
Nivoi su: `debug`, `info`, `warn`, `error`. Pogledajte [SDK dokumentaciju](https://kiyocode.ai/docs/sdk) za detalje.
