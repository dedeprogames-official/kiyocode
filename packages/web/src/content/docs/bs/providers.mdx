---
title: Provajderi
description: Korištenje bilo kojeg LLM provajdera u KiyoCode.
---

import config from "../../../../config.mjs"
export const console = config.console

KiyoCode koristi [AI SDK](https://ai-sdk.dev/) i [Models.dev](https://models.dev) za podršku **75+ LLM provajdera** i podržava pokretanje lokalnih modela.

Za dodavanje provajdera potrebno je:

1. Dodajte API ključeve za provajdera koristeći naredbu `/connect`.
2. Konfigurirajte dobavljača u vašoj KiyoCode konfiguraciji.
...
Možete prilagoditi dobavljače putem odjeljka `provider` u vašem KiyoCode
config.
...
KiyoCode Zen je lista modela koje je obezbedio KiyoCode tim koji su bili
testirano i potvrđeno da dobro radi sa KiyoCode. [Saznajte više](/docs/zen).

:::tip
Ako ste novi, preporučujemo da počnete sa KiyoCode Zen.
:::

1. Pokrenite naredbu `/connect` u TUI-u, odaberite kiyocode i idite na [kiyocode.ai/auth](https://kiyocode.ai/auth).
...
Radi kao i svaki drugi provajder u KiyoCode i potpuno je opcionalan za korištenje.

---

## KiyoCode Go

KiyoCode Go je jeftin plan pretplate koji pruža pouzdan pristup popularnim modelima otvorenog kodiranja koje pruža KiyoCode tim i koji su testirani i verificirani da dobro rade s KiyoCode-om.

1. Pokrenite naredbu `/connect` u TUI-u, odaberite `OpenCode Go` i idite na [kiyocode.ai/auth](https://kiyocode.ai/zen).
...
Da biste koristili Amazon Bedrock s KiyoCode:
...
2. **Konfigurirajte autentifikaciju** koristeći jedan od sljedećih metoda:
   ...
   Postavite jednu od ovih varijabli okruženja dok pokrećete kiyocode:
...
:::info
[Anthropic] (https://anthropic.com) službeno ne podržava korištenje vaše Claude Pro/Max pretplate u KiyoCode.
:::
...
Možete konfigurirati kiyocode za korištenje lokalnih modela preko [Atomic Chata](https://atomic.chat) — desktop aplikacije koja pokreće lokalne LLM-ove iza OpenAI-kompatibilnog API servera (zadana krajnja tačka `http://127.0.0.1:1337/v1`).
...
2. Idite na [Azure AI Foundry](https://ai.azure.com/) i implementirajte model.
   ...
   :::note
   Ime implementacije mora odgovarati imenu modela da bi kiyocode ispravno radio.
   :::
...
Također možete dodati modele kroz svoju kiyocode konfiguraciju.
...
da pohrani token u kiyocode auth memoriju.
:::

##### Samostalni GitLab

:::note[Napomena o usklađenosti]
KiyoCode koristi mali model za neke AI zadatke kao što je generiranje naslova sesije.
Podrazumevano je konfigurisan da koristi gpt-5-nano, a hostuje ga Zen. Da zaključate KiyoCode
...
Da biste koristili svoju GitHub Copilot pretplatu s kiyocode:
...
Za korištenje Google Vertex AI s KiyoCode:
...
2. Postavite potrebne varijable okruženja:
   ...
   Postavite ih dok se pokreće kiyocode.
...
Heliconeova funkcija [Sessions](https://docs.helicone.ai/features/sessions) vam omogućava da grupišete povezane LLM zahtjeve zajedno. Koristite dodatak [opencode-helicone-session](https://github.com/H2Shami/opencode-helicone-session) da automatski prijavite svaki KiyoCode razgovor kao sesiju u Helicone-u.
...
Dodatak ubacuje zaglavlja `Helicone-Session-Id` i `Helicone-Session-Name` u vaše zahtjeve. Na stranici Helicone Sessions, vidjet ćete svaki KiyoCode razgovor naveden kao zasebna sesija.
...
Možete konfigurirati kiyocode za korištenje lokalnih modela putem [llama.cpp's](https://github.com/ggml-org/llama.cpp) uslužnog programa llama-server
...
Možete konfigurirati kiyocode za korištenje lokalnih modela preko LM Studio.
...
Možete konfigurirati kiyocode za korištenje lokalnih modela putem Ollame.

:::tip
Ollama se može automatski konfigurirati za KiyoCode. Pogledajte [Ollama integracijske dokumente](https://docs.ollama.com/integrations/kiyocode) za detalje.
:::
...
Da biste koristili Ollama Cloud s KiyoCode:
...
3. Kopirajte API ključ za korištenje u KiyoCode.
...
6. **Važno**: Prije upotrebe modela oblaka u KiyoCode, morate lokalno povući informacije o modelu:
...
KiyoCode Zen je lista testiranih i verifikovanih modela koju je obezbedio KiyoCode tim. [Saznajte više](/docs/zen).

1. Prijavite se na **<a href={console}>KiyoCode Zen</a>** i kliknite na **Kreiraj API ključ**.

2. Pokrenite naredbu `/connect` i potražite **KiyoCode Zen**.

```txt
   /connect
```

3. Unesite svoj KiyoCode API ključ.
...
Također možete dodati dodatne modele putem vaše kiyocode konfiguracije.
...
5. Također ih možete prilagoditi putem vaše kiyocode konfiguracije. Evo primjera navođenja provajdera
...
Također možete prilagoditi modele kroz svoju kiyocode konfiguraciju. Evo primjera specificiranja redoslijeda usmjeravanja dobavljača.
...
Polja `limit` omogućavaju KiyoCode da shvati koliko vam je konteksta ostalo. Standardni dobavljači ih automatski preuzimaju sa models.dev.
...
2. Za prilagođene provajdere, provjerite KiyoCode konfiguraciju i:
   - Uvjerite se da ID provajdera korišten u naredbi `/connect` odgovara ID-u u vašoj kiyocode konfiguraciji.
