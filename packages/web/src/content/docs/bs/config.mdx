---
title: Konfiguracija
description: Korištenje KiyoCode JSON konfiguracije.
---

Možete konfigurirati KiyoCode koristeći JSON konfiguracijski fajl.

---

## Format

KiyoCode podržava i **JSON** i **JSONC** (JSON sa komentarima) formate.
...
Postavite svoju globalnu KiyoCode konfiguraciju u `~/.config/opencode/opencode.json`. Koristite globalnu konfiguraciju za korisničke preferencije kao što su provajderi, modeli i dozvole.
...
Kada se KiyoCode pokrene, traži konfiguracijsku datoteku u trenutnom direktoriju ili prelazi do najbližeg Git direktorija.
...
Konfiguracijski fajl ima šemu koja je definirana u [**`kiyocode.ai/config.json`**](https://kiyocode.ai/config.json).

TUI konfiguracija koristi [**`kiyocode.ai/tui.json`**](https://kiyocode.ai/tui.json).
...
- `mdns` - Omogući mDNS otkrivanje servisa. Ovo omogućava drugim uređajima na mreži da otkriju vaš KiyoCode server.
...
Možete konfigurirati dobavljače i modele koje želite koristiti u svojoj KiyoCode konfiguraciji kroz opcije `provider`, `model` i `small_model`.
...
Opcija `small_model` konfigurira poseban model za lagane zadatke poput generiranja naslova. Podrazumevano, KiyoCode pokušava da koristi jeftiniji model ako je dostupan od vašeg provajdera, inače se vraća na vaš glavni model.
...
Zadani agent mora biti primarni agent (ne podagent). Ovo može biti ugrađeni agent kao što je `"build"` ili `"plan"`, ili [prilagođeni agent](/docs/agents) koji ste definirali. Ako navedeni agent ne postoji ili je podagent, KiyoCode će se vratiti na `"build"` s upozorenjem.
...
KiyoCode će automatski preuzeti sva nova ažuriranja kada se pokrene. Ovo možete onemogućiti opcijom `autoupdate`.
...
Prema zadanim postavkama, KiyoCode **dopušta sve operacije** bez potrebe za eksplicitnim dopuštenjem. Ovo možete promijeniti koristeći opciju `permission`.
...
[Plugins](/docs/plugins) proširuju KiyoCode sa prilagođenim alatima, kukicama i integracijama.
...
Ovo je korisno kada želite da ograničite KiyoCode da koristi samo određene provajdere umjesto da ih onemogućavate jednog po jednog.
