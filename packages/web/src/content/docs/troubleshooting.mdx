...
To debug issues with KiyoCode, start by checking the logs and local data it stores on disk.
...
kiyocode stores session data and other application data on disk at:
...
To uninstall the KiyoCode CLI and remove its related files, run:
...
To remove KiyoCode Desktop, uninstall the application through your operating system's app management tools.

---

## Desktop app

KiyoCode Desktop runs a local KiyoCode server (the `opencode-cli` sidecar) in the background. Most issues are caused by a misbehaving plugin, a corrupted cache, or a bad server setting.
...
KiyoCode can also load local plugins from disk. Temporarily move these out of the way (or rename the folder) and restart the desktop app:
...
If disabling plugins doesn't help (or a plugin install is stuck), clear the cache so KiyoCode can rebuild it.

1. Quit KiyoCode Desktop completely.
...
3. Restart KiyoCode Desktop.

---

### Fix server connection issues

KiyoCode Desktop can either start its own local server (default) or connect to a server URL you configured.
...
On Windows, KiyoCode Desktop requires the Microsoft Edge **WebView2 Runtime**. If the app opens to a blank window or won't start, install/update WebView2 and try again.

---

### Windows: General performance issues

If you're experiencing slow performance, file access issues, or terminal problems on Windows, try using [WSL (Windows Subsystem for Linux)](/docs/windows-wsl). WSL provides a Linux environment that works more seamlessly with KiyoCode's features.

---

### Notifications not showing

KiyoCode Desktop only shows system notifications when:

- notifications are enabled for KiyoCode in your OS settings, and
...
1. Quit KiyoCode Desktop.
2. Find and delete these files (they live in the KiyoCode Desktop app data directory):
...
If you're experiencing issues with KiyoCode:
...
If you encounter API call errors, this may be due to outdated provider packages. kiyocode dynamically installs provider packages (OpenAI, Anthropic, Google, etc.) as needed and caches them locally.
...
2. Restart kiyocode to reinstall the latest provider packages

This will force kiyocode to download the most recent versions of provider packages, which often resolves compatibility issues with model parameters and API changes.
...
kiyocode will detect if you're using Wayland and prefer `wl-clipboard`, otherwise it will try to find clipboard tools in order of: `xclip` and `xsel`.
