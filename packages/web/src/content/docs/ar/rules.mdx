---
title: القواعد
description: اضبط تعليمات مخصصة لـ kiyocode.
---

يمكنك تزويد kiyocode بتعليمات مخصصة عبر إنشاء ملف `AGENTS.md`. هذا مشابه لقواعد Cursor. يحتوي هذا الملف على تعليمات تُضاف إلى سياق نموذج اللغة (LLM) لتخصيص سلوكه لمشروعك المحدد.

---

## التهيئة

لإنشاء ملف `AGENTS.md` جديد، يمكنك تشغيل الأمر `/init` في kiyocode.

:::tip
يُستحسن عمل commit لملف `AGENTS.md` الخاص بمشروعك إلى Git.
:::

سيقوم ذلك بفحص مشروعك وكل محتوياته لفهم طبيعة المشروع وتوليد ملف `AGENTS.md` بناءً على ذلك. يساعد هذا kiyocode على التنقّل داخل المشروع بشكل أفضل.
...
يدعم kiyocode أيضًا قراءة ملف `AGENTS.md` من مواقع متعددة، ولكل موقع غرض مختلف.
...
يمكنك أيضًا وضع قواعد عامة في ملف `~/.config/opencode/AGENTS.md`. تُطبَّق هذه القواعد على جميع جلسات kiyocode.
...
للمستخدمين الذين ينتقلون من Claude Code، يدعم KiyoCode اصطلاحات ملفات Claude Code كبدائل احتياطية:
...
عند بدء kiyocode، يبحث عن ملفات القواعد بهذا الترتيب:
...
على الرغم من أن kiyocode لا يقوم تلقائيًا بتحليل مراجع الملفات داخل `AGENTS.md`، يمكنك تحقيق وظيفة مشابهة بطريقتين:
...
يمكنك تعليم kiyocode قراءة ملفات خارجية عبر تقديم تعليمات صريحة داخل `AGENTS.md`. إليك مثالًا عمليًا:
...
- ضمان أن kiyocode يحمّل الملفات فقط عند الحاجة للمهمة المحددة
