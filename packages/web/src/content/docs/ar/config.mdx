---
title: الإعدادات
description: استخدام ملف إعدادات KiyoCode بصيغة JSON.
---

يمكنك ضبط KiyoCode باستخدام ملف إعدادات بصيغة JSON.

---

## التنسيق

يدعم KiyoCode تنسيقي **JSON** و**JSONC** (JSON مع تعليقات).
...
ضع إعدادات KiyoCode العامة في `~/.config/opencode/opencode.json`. استخدم الإعدادات العامة للتفضيلات على مستوى المستخدم مثل السمات، والمزوّدين، أو اختصارات المفاتيح.
...
عند تشغيل KiyoCode، يبحث عن ملف إعدادات في الدليل الحالي أو يصعد حتى أقرب دليل Git.
...
يحتوي ملف الإعدادات على مخطط مُعرَّف في [**`kiyocode.ai/config.json`**](https://kiyocode.ai/config.json).
...
- `mdns` - تفعيل اكتشاف الخدمة عبر mDNS. يتيح ذلك للأجهزة الأخرى على الشبكة اكتشاف خادم KiyoCode.
...
يمكنك ضبط المزوّدين والنماذج التي تريد استخدامها في إعدادات KiyoCode عبر الخيارات `provider` و`model` و`small_model`.
...
يضبط الخيار `small_model` نموذجًا منفصلًا للمهام الخفيفة مثل توليد العناوين. افتراضيًا يحاول KiyoCode استخدام نموذج أقل تكلفة إذا كان متاحًا لدى مزوّدك، وإلا فسيعود إلى النموذج الرئيسي.
...
يمكنك ضبط السمة التي تريد استخدامها في إعدادات KiyoCode عبر الخيار `theme`.
...
يجب أن يكون الوكيل الافتراضي وكيلًا أساسيًا (وليس وكيلًا فرعيًا). يمكن أن يكون وكيلًا مدمجًا مثل `"build"` أو `"plan"`، أو [وكيلًا مخصصًا](/docs/agents) قمت بتعريفه. إذا لم يكن الوكيل المحدد موجودًا أو كان وكيلًا فرعيًا، فسيعود KiyoCode إلى `"build"` مع تحذير.
...
سيقوم KiyoCode بتنزيل أي تحديثات جديدة تلقائيًا عند بدء التشغيل. يمكنك تعطيل ذلك عبر الخيار `autoupdate`.
...
افتراضيًا يسمح kiyocode **بجميع العمليات** دون الحاجة إلى موافقة صريحة. يمكنك تغيير ذلك عبر الخيار `permission`.
...
[الإضافات](/docs/plugins) توسّع KiyoCode بأدوات وخُطافات وتكاملات مخصصة.
...
هذا مفيد عندما تريد تقييد KiyoCode لاستخدام مزوّدين محددين بدلًا من تعطيلهم واحدًا تلو الآخر.
