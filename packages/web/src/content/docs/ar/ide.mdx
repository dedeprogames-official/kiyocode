---
title: IDE
description: امتداد KiyoCode لـ VS Code و Cursor وغيرها من بيئات التطوير
---

يتكامل KiyoCode مع VS Code و Cursor أو أي IDE يدعم terminal. ما عليك سوى تشغيل `opencode` في terminal للبدء.

---

## الاستخدام

- **تشغيل سريع**: استخدم `Cmd+Esc` (Mac) أو `Ctrl+Esc` (Windows/Linux) لفتح KiyoCode في عرض terminal مقسّم، أو للتركيز على جلسة terminal موجودة إذا كانت قيد التشغيل بالفعل.
- **جلسة جديدة**: استخدم `Cmd+Shift+Esc` (Mac) أو `Ctrl+Shift+Esc` (Windows/Linux) لبدء جلسة terminal جديدة لـ KiyoCode حتى لو كانت هناك جلسة مفتوحة. يمكنك أيضا النقر على زر KiyoCode في واجهة المستخدم.
- **وعي بالسياق**: شارك تلقائيا تحديدك الحالي أو تبويبك مع KiyoCode.
...
لتثبيت KiyoCode على VS Code والتفرعات الشائعة مثل Cursor و Windsurf و VSCodium:
...
ابحث عن **KiyoCode** في Extension Marketplace وانقر **Install**.
