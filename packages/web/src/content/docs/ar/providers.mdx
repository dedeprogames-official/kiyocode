---
title: المزوّدون
description: استخدام أي موفّر LLM في KiyoCode.
---

import config from "../../../../config.mjs"
export const console = config.console

يستخدم KiyoCode ‏[AI SDK](https://ai-sdk.dev/) و[Models.dev](https://models.dev) لدعم **أكثر من 75 موفّر LLM**، كما يدعم تشغيل النماذج المحلية.

لإضافة مزوّد تحتاج إلى:

1. أضف مفاتيح API الخاصة بالمزوّد باستخدام الأمر `/connect`.
2. اضبط المزوّد في ملف إعدادات KiyoCode.
...
يمكنك تخصيص المزوّدات عبر قسم `provider` في ملف إعدادات KiyoCode.
...
KiyoCode Zen هي قائمة نماذج يوفّرها فريق KiyoCode وقد تم
اختبارها والتحقق من أنها تعمل بشكل جيد مع KiyoCode. [اعرف المزيد](/docs/zen).

:::tip
إذا كنت جديدا، فننصح بالبدء مع KiyoCode Zen.
:::

1. شغّل الأمر `/connect` في TUI، واختر kiyocode، ثم انتقل إلى [kiyocode.ai/auth](https://kiyocode.ai/auth).
...
يعمل مثل أي مزوّد آخر في KiyoCode واستخدامه اختياري بالكامل.

---

## KiyoCode Go

KiyoCode Go هي خطة اشتراك منخفضة التكلفة توفّر وصولا موثوقا إلى نماذج البرمجة المفتوحة الشهيرة المقدّمة من فريق KiyoCode، والتي تم اختبارها والتحقق من أنها تعمل بشكل جيد مع KiyoCode.

1. شغّل الأمر `/connect` في TUI، واختر `OpenCode Go`، ثم انتقل إلى [kiyocode.ai/auth](https://kiyocode.ai/zen).
...
لاستخدام Amazon Bedrock مع KiyoCode:
...
2. **اضبط المصادقة** باستخدام إحدى الطرق التالية:
   ...
   عيّن أحد متغيرات البيئة التالية أثناء تشغيل kiyocode:
...
:::info
استخدام اشتراك Claude Pro/Max داخل KiyoCode غير مدعوم رسميا من [Anthropic](https://anthropic.com).
:::
...
يمكنك تكوين kiyocode لاستخدام النماذج المحلية عبر [Atomic Chat](https://atomic.chat)، وهو تطبيق سطح مكتب يقوم بتشغيل LLMs المحلية خلف خادم API متوافق مع OpenAI (نقطة النهاية الافتراضية `http://127.0.0.1:1337/v1`).
...
2. اذهب إلى [Azure AI Foundry](https://ai.azure.com/) وانشر نموذجا.
   ...
   :::note
   يجب أن يطابق اسم النشر اسم النموذج كي يعمل kiyocode بشكل صحيح.
   :::
...
5. شغّل الأمر `/models` لاختيار نموذج.
   ...
   يمكنك أيضا إضافة نماذج عبر إعدادات kiyocode.
...
تخزين الرمز في مساحة تخزين مصادقة kiyocode.
:::

##### GitLab مستضاف ذاتيا

:::note[ملاحظة امتثال]
يستخدم KiyoCode نموذجا صغيرا لبعض مهام الذكاء الاصطناعي مثل إنشاء عنوان الجلسة.
تم ضبطه لاستخدام gpt-5-nano افتراضيا، مستضافا عبر Zen. لحصر KiyoCode
...
لاستخدام اشتراك GitHub Copilot مع kiyocode:
...
لاستخدام Google Vertex AI مع KiyoCode:
...
إذا رأيت ميزة أو نموذجا في Helicone لم يتم ضبطه تلقائيا عبر kiyocode، فيمكنك دائما ضبطه بنفسك.
...
تتيح ميزة [Sessions](https://docs.helicone.ai/features/sessions) في Helicone تجميع طلبات LLM المرتبطة معا. استخدم ملحق [opencode-helicone-session](https://github.com/H2Shami/opencode-helicone-session) لتسجيل كل محادثة KiyoCode تلقائيا كجلسة في Helicone.
...
يقوم الملحق بحقن ترويسات `Helicone-Session-Id` و`Helicone-Session-Name` ضمن طلباتك. في صفحة Sessions داخل Helicone، سترى كل محادثة KiyoCode مدرجة كجلسة مستقلة.
...
يمكنك ضبط kiyocode لاستخدام نماذج محلية عبر أداة llama-server ضمن [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
يمكنك ضبط kiyocode لاستخدام نماذج محلية عبر LM Studio.
...
يمكنك ضبط kiyocode لاستخدام نماذج محلية عبر Ollama.

:::tip
يمكن لـ Ollama ضبط نفسه تلقائيا للعمل مع KiyoCode. راجع [توثيق تكامل Ollama](https://docs.ollama.com/integrations/kiyocode) للتفاصيل.
:::
...
لاستخدام Ollama Cloud مع KiyoCode:
...
3. انسخ مفتاح API لاستخدامه في KiyoCode.
...
6. **مهم**: قبل استخدام نماذج السحابة في KiyoCode، يجب سحب معلومات النموذج محليا:
...
KiyoCode Zen هي قائمة من النماذج التي تم اختبارها والتحقق منها مقدّمة من فريق KiyoCode. [اعرف المزيد](/docs/zen).

1. سجّل الدخول إلى **<a href={console}>KiyoCode Zen</a>** وانقر **Create API Key**.

2. شغّل الأمر `/connect` وابحث عن **KiyoCode Zen**.

   ```txt
   /connect
   ```

3. أدخل مفتاح API الخاص بـ KiyoCode.
...
4. يتم تحميل العديد من نماذج OpenRouter مسبقا بشكل افتراضي؛ شغّل الأمر `/models` لاختيار ما تريده.
   ...
   يمكنك أيضا إضافة نماذج إضافية عبر إعدادات kiyocode.
...
5. يمكنك أيضا تخصيصها عبر إعدادات kiyocode. إليك مثالا على تحديد مزوّد
...
لاستخدام [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) مع Kiyocode:
...
يمكنك أيضا تخصيص النماذج عبر إعدادات kiyocode. إليك مثالا على تحديد ترتيب توجيه المزوّدات.
...
:::tip
يمكنك استخدام أي مزوّد OpenAI-compatible مع kiyocode. معظم مزوّدي الذكاء الاصطناعي الحديثة يقدّمون APIs متوافقة مع OpenAI.
:::
...
تسمح حقول `limit` لـ KiyoCode بفهم مقدار السياق المتبقي لديك. تسحب المزوّدات القياسية هذه القيم تلقائيا من models.dev.
...
2. للمزوّدات المخصّصة، راجع إعدادات kiyocode وتأكد من:
   - أن provider ID المستخدم في الأمر `/connect` يطابق الـ ID في إعدادات kiyocode.
