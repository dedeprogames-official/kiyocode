---
title: TUI
description: Using the KiyoCode terminal user interface.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

KiyoCode provides an interactive terminal interface or TUI for working on your projects with an LLM.

Running KiyoCode starts the TUI for the current directory.
...
When using the KiyoCode TUI, you can type `/` followed by a command name to quickly execute actions. For example:
...
Add a provider to KiyoCode. Allows you to select from available providers and add their API keys.
...
Exit KiyoCode. _Aliases_: `/quit`, `/q`
...
- `leader_timeout` - Controls how long KiyoCode waits after the leader key. Defaults to `2000`.
