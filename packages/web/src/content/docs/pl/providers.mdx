---
title: Dostawcy
description: Korzystanie z dowolnego dostawcy LLM w kiyocode.
---

import config from "../../../../config.mjs"
export const console = config.console

kiyocode używa [AI SDK](https://ai-sdk.dev/) i [Models.dev](https://models.dev) do obsługi **ponad 75 dostawców LLM** i obsługuje uruchamianie modeli lokalnych.

Aby dodać dostawcę należy:

1. Dodaj klucze API dla dostawcy za pomocą komendy `/connect`.
2. Skonfiguruj dostawcę w konfiguracji kiyocode.
...
Możesz dostosować dostawców za pomocą sekcji `provider` w swoim kiyocode
config.
...
KiyoCode Zen to lista modeli dostarczonych przez zespół kiyocode, które zostały
przetestowane i zweryfikowane, aby dobrze współpracować z kiyocode. [Dowiedz się więcej](/docs/zen).

:::tip
Jeśli jesteś nowy, zalecamy rozpoczęcie od KiyoCode Zen.
:::

1. Uruchom polecenie `/connect` w TUI, wybierz `OpenCode Zen` i przejdź do [kiyocode.ai/auth](https://kiyocode.ai/zen).
...
Działa jak każdy inny dostawca w kiyocode i jest całkowicie opcjonalny w użyciu.

---

## KiyoCode Go

KiyoCode Go to tani plan subskrypcji, który zapewnia niezawodny dostęp do popularnych modeli open coding dostarczanych przez zespół kiyocode, które zostały przetestowane i zweryfikowane pod kątem dobrej współpracy z kiyocode.

1. Uruchom polecenie `/connect` w TUI, wybierz `OpenCode Go` i przejdź do [kiyocode.ai/auth](https://kiyocode.ai/zen).
...
Aby używać Amazon Bedrock z kiyocode:
...
2. **Skonfiguruj uwierzytelnianie** przy użyciu jednej z następujących metod:
   ...
   Ustaw jedną z tych zmiennych środowiskowych podczas uruchamiania kiyocode:
...
:::info
Korzystanie z subskrypcji Claude Pro/Max w kiyocode nie jest oficjalnie obsługiwane przez [Anthropic](https://anthropic.com).
:::
...
Możesz skonfigurować kiyocode do korzystania z modeli lokalnych przez [Atomic Chat](https://atomic.chat) — aplikację desktopową uruchamiającą lokalne LLM-y za serwerem API zgodnym z OpenAI (domyślny endpoint `http://127.0.0.1:1337/v1`).
...
2. Przejdź do [Azure AI Foundry](https://ai.azure.com/) i wdróż model.
   ...
   :::note
   Aby kod kiyocode działał poprawnie, nazwa wdrożenia musi być zgodna z nazwą modelu.
   :::
...
5. Uruchom komendę `/models`, aby wybrać model.
   ...
   Możesz także dodawać modele za pomocą konfiguracji kiyocode.
...
kiyocode używa małego modelu do niektórych zadań AI, takich jak generowanie tytułu sesji.
Domyślnie jest skonfigurowany do używania gpt-5-nano, hostowanego przez Zen. Aby zablokować kiyocode
...
Aby korzystać z subskrypcji GitHub Copilot z kodem kiyocode:
...
Aby używać Google Vertex AI z kiyocode:
...
2. Ustaw wymagane zmienne środowiskowe:
   ...
   Ustaw je podczas uruchamiania kiyocode.
...
Jeśli zobaczysz funkcję lub model firmy Helicone, która nie jest konfigurowana automatycznie za pomocą kiyocode, zawsze możesz ją skonfigurować samodzielnie.
...
Funkcja [Sesje](https://docs.helicone.ai/features/sessions) firmy Helicone umożliwia grupowanie powiązanych żądań LLM. Użyj wtyczki [opencode-helicone-session](https://github.com/H2Shami/opencode-helicone-session), aby automatycznie rejestrować każdą konwersację kiyocode jako sesję w Helicone.
...
Wtyczka wstawia nagłówki `Helicone-Session-Id` i `Helicone-Session-Name` do Twoich żądań. Na stronie Sesje Helicone każda konwersacja kiyocode będzie wymieniona jako osobna sesja.
...
Możesz skonfigurować kiyocode tak, aby korzystał z modeli lokalnych, za pomocą narzędzia llama-server [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
Możesz skonfigurować kiyocode tak, aby korzystał z modeli lokalnych poprzez LM Studio.
...
Możesz skonfigurować kiyocode tak, aby korzystał z modeli lokalnych poprzez Ollamę.

:::tip
Ollama może automatycznie skonfigurować się pod kątem kiyocode. Aby uzyskać szczegółowe informacje, zobacz [dokumentację integracji Ollama](https://docs.ollama.com/integrations/kiyocode).
:::
...
Aby korzystać z Ollama Cloud z kiyocode:
...
3. Skopiuj klucz API do użycia w kiyocode.
...
6. **Ważne**: Przed użyciem modeli chmurowych w kiyocode musisz pobrać informacje o modelu lokalnie:
...
KiyoCode Zen to lista przetestowanych i zweryfikowanych modeli udostępniona przez zespół kiyocode. [Dowiedz się więcej](/docs/zen).

1. Zaloguj się do **<a href={console}>KiyoCode Zen</a>** i kliknij **Utwórz klucz API**.

2. Uruchom polecenie `/connect` i wyszukaj **KiyoCode Zen**.

   ```txt
   /connect
   ```

3. Wprowadź klucz API kiyocode.
...
4. Wiele modeli OpenRouter jest domyślnie załadowanych fabrycznie. Uruchom komendę `/models`, aby wybrać ten, który chcesz.
   ...
   Możesz także dodać dodatkowe modele za pomocą konfiguracji kiyocode.
...
5. Możesz je także dostosować za pomocą konfiguracji kiyocode. Oto przykład określenia dostawcy
...
Aby używać [generatywnych interfejsów API Scaleway](https://www.scaleway.com/en/docs/generative-apis/) z kiyocode:
...
Możesz także dostosować modele za pomocą konfiguracji kiyocode. Oto przykład określenia kolejności routingu dostawcy.
...
:::tip
Możesz użyć dowolnego dostawcy kompatybilnego z OpenAI z kiyocode. Większość nowoczesnych dostawców sztucznej inteligencji oferuje interfejsy API kompatybilne z OpenAI.
:::
...
Pola `limit` pozwalają kiyocode zrozumieć, ile kontekstu pozostało. Standardowi dostawcy pobierają je automatycznie z models.dev.
...
2. W przypadku dostawców niestandardowych sprawdź konfigurację kiyocode i:
   - Upewnij się, że identyfikator dostawcy użyty w poleceniu `/connect` jest zgodny z identyfikatorem w konfiguracji kiyocode.
