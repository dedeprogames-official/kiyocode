---
title: Wtyczki
description: Napisz własną wtyczkę, aby rozszerzyć kiyocode.
---

Wtyczki umożliwiające wyłączenie kiyocode poprzez podjęcie różnych zdarzeń i zachowanie. Można utworzyć wtyczkę, aby sprawdzić nowe funkcje, integrować się z usługami lub sprawdzaniem zachowania kiyocode.
...
kiyocode uruchomienie `bun install` przy uruchomieniu, aby je uruchomić. Twoje dodatkowe funkcje będą dostępne po zaimportowaniu.
...
- `client`: Klient SDK z kiyocode do sztucznej inteligencji.
...
Oto kilka dostępnych wtyczek, które można udostępnić do kiyocode.
...
:::note
Jeśli korzystasz z aplikacji komputerowej kiyocode, może ona automatycznie wysłać wiadomość systemową, gdy odpowiedź jest gotowa lub gdy wystąpią błędy sesji.
:::
...
Zablokuj kiyocode czytanie plików `.env`:
...
Wtyczki mogą również dodawać niestandardowe narzędzia do kiyocode:
...
Pomocnik `tool` tworzy narzędzie użytkowe, które może wywołać kod kiyocode. Pobierane schematu Zoda i narzędzie wyszukiwania za pomocą:
...
Poziomy: `debug`, `info`, `warn`, `error`. Aby uzyskać szczegółowe informacje, zobacz [dokumentację pakietu SDK](https://kiyocode.ai/docs/sdk).
