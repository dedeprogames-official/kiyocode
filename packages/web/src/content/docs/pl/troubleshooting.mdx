...
Aby debugować problemy z kiyocode, zacznij od sprawdzenia dzienników i danych lokalnych przechowywanych na dysku.
...
kiyocode przechowuje dane sesji i inne dane aplikacji na dysku pod adresem:
...
kiyocode Desktop uruchamia w tle lokalny serwer kiyocode (sidecar `opencode-cli`). Większość problemów jest spowodowana nieprawidłowo działającą wtyczką, uszkodzoną pamięcią podręczną lub złymi ustawieniami serwera.
...
kiyocode może także ładować lokalne wtyczki z dysku. Tymczasowo usuń je (lub zmień nazwę folderu) i uruchom ponownie aplikację komputerową:
...
Jeśli wyłączenie wtyczek nie pomoże (lub instalacja wtyczki utknęła), wyczyść pamięć podręczną, aby kiyocode mógł ją odbudować.

1. Całkowicie zamknij kiyocode Desktop.
...
3. Uruchom ponownie kiyocode Desktop.

---

### Napraw problemy z połączeniem z serwerem

kiyocode Desktop może uruchomić własny serwer lokalny (domyślnie) lub połączyć się ze skonfigurowanym adresem URL serwera.
...
W systemie Windows kiyocode Desktop wymaga Microsoft Edge **WebView2 Runtime**. Jeśli aplikacja otwiera się w pustym oknie lub nie uruchamia się, zainstaluj/zaktualizuj WebView2 i spróbuj ponownie.

---

### Windows: Ogólne problemy z wydajnością

Jeśli doświadczasz niskiej wydajności, problemów z dostępem do plików lub problemów z terminalem w systemie Windows, spróbuj użyć [WSL (podsystem Windows dla systemu Linux) (./windows-wsl). WSL zapewnia środowisko Linux, które płynniej współpracuje z funkcjami kiyocode.

---

### Brak powiadomień

kiyocode Desktop pokazuje powiadomienia systemowe tylko wtedy, gdy:

- powiadomienia są włączone dla kiyocode w ustawieniach systemu operacyjnego, oraz
...
1. Zamknij całkowicie kiyocode Desktop.
2. Znajdź i usuń te pliki (znajdują się w katalogu danych aplikacji kiyocode Desktop):
...
Jeśli masz problemy z kiyocode:
...
Jeśli napotkasz błędy wywołań API, może to wynikać z nieaktualnych pakietów dostawców. kiyocode dynamicznie instaluje pakiety dostawców (OpenAI, Anthropic, Google itp.) w razie potrzeby i przechowuje je lokalnie w pamięci podręcznej.
...
2. Uruchom ponownie kod kiyocode, aby ponownie zainstalować najnowsze pakiety dostawców

Zmusi to kiyocode do pobrania najnowszych wersji pakietów dostawców, co często rozwiązuje problemy ze zgodnością z parametrami modelu i zmianami API.
...
kiyocode wykryje, czy używasz Waylanda i wolisz `wl-clipboard`, w przeciwnym razie spróbuje znaleźć narzędzia schowka w kolejności: `xclip` i `xsel`.
