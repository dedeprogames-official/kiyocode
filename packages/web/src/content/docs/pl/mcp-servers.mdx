...
Możesz dodać dodatkowe informacje do kiyocode za pomocą _Model Context Protocol_, w skrócie MCP. kiyocode obsługuje zarówno serwery lokalne, jak i zdalne.
...
Dostępny serwery MCP w [kiyocode Config](https://kiyocode.ai/docs/config/) w `mcp`. Dodaj każdy MCP z unikalną nazwą. Możesz zgłosić się do tego MCP po nazwie, pytając LLM.
...
kiyocode automatyczne uwierzytelnianie OAuth dla zdalnych serwerów MCP. Gdy serwer wymaga uwierzytelnienia, kiyocode:
...
Jeśli serwer wymaga uwierzytelnienia, kiyocode poprosi Cię o uwierzytelnienie przy próbie jego użycia. Jeśli nie, możesz [ręcznie podłączyć przepływ] (#authenticating) za pomocą `opencode mcp auth <server-name>`.
...
Komenda `mcp auth` została udostępniona w celu autoryzacji. Po autoryzacji kiyocode bezpieczne przechowa tokeny w `~/.local/share/opencode/mcp-auth.json`.
...
Twoje MCP są dostępne jako narzędzie w kiyocode, obok narzędzi dodatkowych. Można to zrobić poprzez podłączenie kiyocode, jak również w innym przypadku.
...
Spowoduje otwarcie okna konfiguracji, w którym można zakończyć proces OAuth i połączyć kiyocode z kontem Sentry.
