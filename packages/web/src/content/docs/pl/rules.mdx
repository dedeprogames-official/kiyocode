---
title: Zasady
description: Ustaw niestandardowe instrukcje dla kiyocode.
---

Możesz dostarczyć niestandardowe instrukcje do kiyocode, tworząc plik `AGENTS.md`. Jest to podobne do zasad Cursora. Zawiera instrukcje, które zostaną uwzględnione w kontekście LLM, aby dostosować jego zachowanie do konkretnego projektu.
...
kiyocode obsługuje także odczytywanie pliku `AGENTS.md` z wielu lokalizacji. A to służy różnym celom.
...
Możesz także mieć reguły globalne w pliku `~/.config/opencode/AGENTS.md`. Jest to stosowane we wszystkich sesjach kiyocode.
...
W przypadku użytkowników migrujących z Claude Code kiyocode obsługuje konwencje plików Claude Code jako rozwiązania awaryjne:
...
Po uruchomieniu kiyocode szuka plików reguł w następującej kolejności:
...
Chociaż kiyocode nie analizuje automatycznie odniesień do plików w `AGENTS.md`, możesz osiągnąć podobną funkcjonalność na dwa sposoby:
...
Możesz nauczyć kiyocode czytać pliki zewnętrzne, podając wyraźne instrukcje w swoim `AGENTS.md`. Oto praktyczny przykład:
...
- Upewnij się, że kiyocode ładuje pliki tylko wtedy, gdy są potrzebne do konkretnego zadania
