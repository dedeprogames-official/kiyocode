---
title: Web
description: Korzystanie z kiyocode w przeglądarce.
---

kiyocode może działać jako aplikacja internetowa w przeglądarce, zapewniając takie same możliwości kodowania AI bez konieczności korzystania z terminala.

![Sieć kiyocode - Nowa sesja](../../../assets/web/web-homepage-new-session.png)
...
Spowoduje to uruchomienie lokalnego serwera na `127.0.0.1` z losowo dostępnym portem i automatyczne otwarcie kiyocode w domyślnej przeglądarce.
...
Domyślnie kiyocode wybiera dostępny port. Możesz określić port:
...
Domyślnie serwer łączy się z `127.0.0.1` (tylko localhost). Aby udostępnić kiyocode w swojej sieci:

```bash
opencode web --hostname 0.0.0.0
```

Podczas korzystania z `0.0.0.0` kiyocode wyświetli zarówno adresy lokalne, jak i sieciowe:
...
Po uruchomieniu interfejs sieciowy zapewnia dostęp do sesji kiyocode.

### Sesje

Przeglądaj sesje i zarządzaj nimi ze strony głównej. Możesz zobaczyć aktywne sesje i rozpocząć nowe.

![kiyocode Web - Aktywna sesja](../../../assets/web/web-homepage-active-session.png)
