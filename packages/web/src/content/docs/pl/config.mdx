---
title: Konfiguracja
description: Ustawienia KiyoCode w pliku JSON.
---

Możesz dostosować KiyoCode za pomocą pliku konfiguracyjnego JSON.

---

## Format

KiyoCode obsługuje formaty **JSON** i **JSONC** (JSON z komentarzami).
...
Umieść swoją globalną konfigurację KiyoCode w `~/.config/opencode/opencode.json`. Użyj jej do ustawień ogólnych dla użytkownika, takich jak dostawcy, modele i uprawnienia.
...
Kiedy KiyoCode się uruchamia, szuka pliku konfiguracyjnego w katalogu głównym repozytorium Git.
...
Plik konfiguracyjny ma schemat JSON dostępny pod adresem [**`kiyocode.ai/config.json`**](https://kiyocode.ai/config.json).
...
- `mdns` - Włącz wykrywanie usług mDNS. Dzięki temu inne urządzenia w sieci mogą łatwo znaleźć Twój serwer KiyoCode.
...
Opcja `small_model` konfiguruje oddzielny model do lżejszych zadań, takich jak generowanie tytułów. Domyślnie KiyoCode wybiera tańszy model od tego samego dostawcy, jeśli jest dostępny, w przeciwnym razie powraca do modelu głównego.
...
Agent domyślny musi być agentem typu `primary` (nie `subagent`). Może to być agent wbudowany, taki jak `"build"` lub `"plan"`, albo zdefiniowany przez Ciebie [agent niestandardowy](/docs/agents). Jeśli agent nie istnieje lub jest subagentem, KiyoCode powróci do `"build"` z ostrzeżeniem.
...
KiyoCode może automatycznie pobierać nowe wersje. Możesz to kontrolować za pomocą opcji `autoupdate`.
...
Działa to tylko wtedy, gdy KiyoCode nie został zainstalowany przez menedżera pakietów, takiego jak Homebrew.
...
Domyślnie KiyoCode **zezwala na wszystkie działania** bez konieczności zatwierdzania. Możesz to zmienić za pomocą opcji `permission`.
...
[Wtyczki](/docs/plugins) rozszerzają KiyoCode o niestandardowe narzędzia, hooki i integracje.
...
Jest to przydatne, gdy chcesz ograniczyć użycie KiyoCode tylko do określonych dostawców.
