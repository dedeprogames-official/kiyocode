---
title: Regeln
description: Lege eigene Anweisungen fuer kiyocode fest.
---

Du kannst kiyocode ueber eine `AGENTS.md`-Datei mit eigenen Anweisungen steuern.
...
Um eine neue `AGENTS.md` zu erstellen, fuehre in kiyocode den Befehl `/init` aus.
...
So kann kiyocode sich in deinem Code besser orientieren.

Wenn bereits eine `AGENTS.md` existiert, versucht kiyocode sie zu erweitern.
...
kiyocode kann `AGENTS.md` aus mehreren Orten lesen.
...
Wenn du von Claude Code kommst, unterstuetzt KiyoCode dessen Dateikonventionen als Fallback:
...
Beim Start sucht kiyocode in dieser Reihenfolge nach Regeldateien:
...
kiyocode parst Datei-Referenzen in `AGENTS.md` nicht automatisch.
...
Du kannst kiyocode explizit anweisen, externe Dateien zu lesen.
