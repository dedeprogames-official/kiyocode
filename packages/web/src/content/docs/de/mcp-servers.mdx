...
Sie können externe Tools zu KiyoCode hinzufügen, indem Sie das _Model Context Protocol_ oder MCP verwenden. KiyoCode unterstützt sowohl lokale als auch Remote-Server.
...
Sie können den MCP-Server in Ihrem [KiyoCode Config](https://kiyocode.ai/docs/config/) unter `mcp` definieren. Fügen Sie jedem MCP einen eindeutigen Namen hinzu. Sie können bei der Eingabe von LLM namentlich auf diesen MCP verweisen.
...
KiyoCode übernimmt automatisch die OAuth-Authentifizierung für Remote-MCP-Server. Wenn ein Server eine Authentifizierung erfordert, wird KiyoCode Folgendes tun:
...
Wenn der Server eine Authentifizierung erfordert, werden Sie von KiyoCode beim ersten Versuch, ihn zu verwenden, zur Authentifizierung autorisiert. Wenn nicht, können Sie [manually trigger the flow](#authenticating) mit `opencode mcp auth <server-name>` verwenden.
...
Der Befehl `mcp auth` öffnet Ihren Browser zur Autorisierung. Nach der Autorisierung speichert KiyoCode die Token sicher in `~/.local/share/opencode/mcp-auth.json`.
...
Ihre MCPs sind neben integrierten Tools auch als Tools in KiyoCode verfügbar. Sie können sie also wie jedes andere Tool über die KiyoCode-Konfiguration verwalten.
...
Dadurch wird ein Browserfenster geöffnet, um den OAuth-Ablauf abzuschließen und KiyoCode mit Ihrem Sentry-Konto zu verbinden.
