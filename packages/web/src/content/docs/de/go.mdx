...
KiyoCode Go ist ein kostengünstiges Abonnement — **5 $ für deinen ersten Monat**, danach **10 $/Monat** —, das dir zuverlässigen Zugriff auf beliebte offene Coding-Modelle bietet.

Go funktioniert wie jeder andere Provider in KiyoCode. Du abonnierst KiyoCode Go und
erhältst deinen API-Key. Es ist **völlig optional** und du musst es nicht nutzen, um
KiyoCode zu verwenden.
...
KiyoCode Go bietet dir Zugriff auf diese Modelle für **5 $ im ersten Monat**, danach **10 $/Monat**.

---

## Wie es funktioniert

KiyoCode Go funktioniert wie jeder andere Provider in KiyoCode.

1. Du meldest dich bei **<a href={console}>KiyoCode Zen</a>** an, abonnierst Go und kopierst deinen API-Key.
...
:::note
Nur ein Mitglied pro Workspace kann KiyoCode Go abonnieren.
:::
...
KiyoCode Go beinhaltet die folgenden Limits:
...
Die [Modell-ID](/docs/config/#models) in deiner KiyoCode Config verwendet das Format `opencode-go/<model-id>`. Für Kimi K2.7 Code würdest du beispielsweise `opencode-go/kimi-k2.7-code` in deiner Config verwenden.
...
Wir haben KiyoCode Go entwickelt, um:
...
4. **Keinen Lock-in-Effekt** zu haben, indem wir dir ermöglichen, auch jeden anderen Provider mit KiyoCode zu nutzen.
