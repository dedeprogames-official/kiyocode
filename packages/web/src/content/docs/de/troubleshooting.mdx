...
Wenn KiyoCode Probleme macht, starte mit Logs und lokal gespeicherten Daten auf der Festplatte.
...
kiyocode speichert Sitzungs- und App-Daten auf der Festplatte unter:
...
KiyoCode Desktop startet im Hintergrund einen lokalen KiyoCode-Server (`opencode-cli`-Sidecar).
...
KiyoCode kann auch lokale Plugins von der Festplatte laden. Verschiebe diese voruebergehend (oder benenne den Ordner um) und starte die Desktop-App neu:
...
Wenn das Deaktivieren von Plugins nicht hilft (oder eine Plugin-Installation haengt), leere den Cache, damit KiyoCode ihn neu aufbauen kann.

1. Beende KiyoCode Desktop komplett.
...
3. Starte KiyoCode Desktop neu.

---

### Server-Verbindungsprobleme beheben

KiyoCode Desktop kann entweder einen eigenen lokalen Server starten (Standard) oder sich mit einer konfigurierten Server-URL verbinden.
...
Unter Windows benoetigt KiyoCode Desktop die Microsoft Edge **WebView2 Runtime**. Wenn die App ein leeres Fenster zeigt oder nicht startet, installiere/aktualisiere WebView2 und versuche es erneut.

---

### Windows: Allgemeine Performance-Probleme

Wenn du langsame Performance, Dateizugriffsprobleme oder Terminal-Probleme unter Windows hast, versuche [WSL (Windows Subsystem for Linux)](/docs/windows-wsl). WSL bietet eine Linux-Umgebung, die nahtloser mit KiyoCode-Features funktioniert.

---

### Benachrichtigungen werden nicht angezeigt

KiyoCode Desktop zeigt Systembenachrichtigungen nur wenn:

- Benachrichtigungen fuer KiyoCode in den OS-Einstellungen aktiviert sind, und
...
1. Beende KiyoCode Desktop.
2. Finde und loesche diese Dateien (im App-Data-Verzeichnis von KiyoCode Desktop):
...
Wenn du Probleme mit KiyoCode hast:
...
Wenn du API-Call-Fehler erhaeltst, kann das an veralteten Provider-Paketen liegen. kiyocode installiert Provider-Pakete (OpenAI, Anthropic, Google, etc.) dynamisch bei Bedarf und cached sie lokal.
...
2. Starte kiyocode neu, um die neuesten Provider-Pakete zu installieren

Dies zwingt kiyocode, die neuesten Versionen der Provider-Pakete herunterzuladen, was oft Kompatibilitaetsprobleme mit Modellparametern und API-Aenderungen loest.
...
kiyocode erkennt, ob du Wayland nutzt und bevorzugt `wl-clipboard`, sonst versucht es `xclip` und `xsel` (in dieser Reihenfolge).
