---
title: Anbieter
description: Verwendung eines beliebigen LLM-Anbieters in KiyoCode.
---

import config from "../../../../config.mjs"
export const console = config.console

KiyoCode verwendet [AI SDK](https://ai-sdk.dev/) und [Models.dev](https://models.dev) zur Unterstützung von **75+ LLM-Anbietern** und unterstützt die Ausführung lokaler Modelle.

Um einen Anbieter hinzuzufügen, müssen Sie:

1. Fügen Sie die API-Schlüssel für den Anbieter mit dem Befehl `/connect` hinzu.
2. Konfigurieren Sie den Anbieter in Ihrer KiyoCode-Konfiguration.
...
Sie können die Anbieter über den Abschnitt `provider` in Ihrem KiyoCode anpassen
config.
...
KiyoCode Zen ist eine Liste von Modellen, die vom KiyoCode-Team bereitgestellt wurden
Getestet und verifiziert, dass es gut mit KiyoCode funktioniert. [Learn more](/docs/zen).

:::tip
Wenn Sie neu sind, empfehlen wir Ihnen, mit KiyoCode Zen zu beginnen.
:::

1. Führen Sie den Befehl `/connect` in TUI aus, wählen Sie KiyoCode aus und gehen Sie zu [kiyocode.ai/auth](https://kiyocode.ai/auth).
...
Es funktioniert wie jeder andere Anbieter in KiyoCode und ist völlig optional.

---

## KiyoCode Go

KiyoCode Go ist ein kostenguenstiges Abonnement, das zuverlaessigen Zugriff auf beliebte Open-Coding-Modelle bietet, die vom KiyoCode-Team getestet und verifiziert wurden, dass sie gut mit KiyoCode funktionieren.

1. Führen Sie den Befehl `/connect` in der TUI aus, waehlen Sie `OpenCode Go` und gehen Sie zu [kiyocode.ai/auth](https://kiyocode.ai/zen).
...
So verwenden Sie Amazon Bedrock mit KiyoCode:
...
2. **Konfigurieren Sie die Authentifizierung** mit einer der folgenden Methoden:
   ...
   Legen Sie eine dieser Umgebungsvariablen fest, während Sie KiyoCode ausführen:
...
:::info
Die Verwendung Ihres Claude Pro/Max-Abonnements in KiyoCode wird von [Anthropic](https://anthropic.com) nicht offiziell unterstützt.
:::
...
Sie können kiyocode so konfigurieren, dass es lokale Modelle über [Atomic Chat](https://atomic.chat) verwendet — eine Desktop-Anwendung, die lokale LLMs hinter einem OpenAI-kompatiblen API-Server bereitstellt (Standard-Endpunkt `http://127.0.0.1:1337/v1`).
...
2. Gehen Sie zu [Azure AI Foundry](https://ai.azure.com/) und stellen Sie ein Modell bereit.
   ...
   :::note
   Der Bereitstellungsname muss mit dem Modellnamen übereinstimmen, damit KiyoCode ordnungsgemäß funktioniert.
   :::
...
5. Führen Sie den Befehl `/models` aus, um ein Modell auszuwählen.
   ...
   Sie können Modelle auch über Ihre KiyoCode-Konfiguration hinzufügen.
...
um das Token im KiyoCode-Authentifizierungsspeicher zu speichern.
:::

##### Selbstgehostetes GitLab

:::note[Compliance-Hinweis]
KiyoCode verwendet ein kleines Modell für einige AI-Aufgaben wie das Generieren des Sitzungstitels.
Es ist standardmäßig für die Verwendung von gpt-5-nano konfiguriert und wird von Zen gehostet. Zum Sperren von KiyoCode
...
So verwenden Sie Ihr GitHub Copilot-Abonnement mit KiyoCode:
...
So verwenden Sie Google Vertex AI mit KiyoCode:
...
2. Legen Sie die erforderlichen Umgebungsvariablen fest:
   ...
   Legen Sie sie fest, während Sie KiyoCode ausführen.
...
Falls Sie eine Funktion oder ein Modell von Helicone sehen, das nicht automatisch über KiyoCode konfiguriert wird, können Sie es jederzeit selbst konfigurieren.
...
Mit der [Sessions](https://docs.helicone.ai/features/sessions)-Funktion von Helicone können Sie zusammengehörige LLM-Anfragen gruppieren. Verwenden Sie das [opencode-helicone-session](https://github.com/H2Shami/opencode-helicone-session)-Plugin, um jede KiyoCode-Konversation automatisch als Sitzung in Helicone zu protokollieren.
...
Das Plugin fügt `Helicone-Session-Id`- und `Helicone-Session-Name`-Header in Ihre Anfragen ein. Auf der Seite „Sitzungen“ von Helicone wird jede KiyoCode-Konversation als separate Sitzung aufgeführt.
...
Sie können KiyoCode für die Verwendung lokaler Modelle über das Dienstprogramm [llama.cpp's](https://github.com/ggml-org/llama.cpp) llama-server konfigurieren
...
Sie können KiyoCode über LM Studio für die Verwendung lokaler Modelle konfigurieren.
...
Sie können KiyoCode über Ollama für die Verwendung lokaler Modelle konfigurieren.

:::tip
Ollama kann sich automatisch für KiyoCode konfigurieren. Weitere Informationen finden Sie unter [Ollama integration docs](https://docs.ollama.com/integrations/kiyocode).
:::
...
So verwenden Sie Ollama Cloud mit KiyoCode:
...
3. Kopieren Sie den API-Schlüssel zur Verwendung in KiyoCode.
...
6. **Wichtig**: Bevor Sie Cloud-Modelle in KiyoCode verwenden, müssen Sie die Modellinformationen lokal abrufen:
...
KiyoCode Zen ist eine Liste getesteter und verifizierter Modelle, die vom KiyoCode-Team bereitgestellt werden. [Learn more](/docs/zen).

1. Melden Sie sich bei **<a href={console}>KiyoCode Zen</a>** an und klicken Sie auf **Create API Key**.

2. Führen Sie den Befehl `/connect` aus und suchen Sie nach **KiyoCode Zen**.

   ```txt
   /connect
   ```

3. Geben Sie Ihren KiyoCode API-Schlüssel ein.
...
4. Viele OpenRouter-Modelle sind standardmäßig vorinstalliert. Führen Sie den Befehl `/models` aus, um das gewünschte Modell auszuwählen.
   ...
   Sie können auch zusätzliche Modelle über Ihre KiyoCode-Konfiguration hinzufügen.
...
5. Sie können sie auch über Ihre KiyoCode-Konfiguration anpassen. Hier ist ein Beispiel für die Angabe eines Anbieters
...
So verwenden Sie [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) mit KiyoCode:
...
Sie können Modelle auch über Ihre KiyoCode-Konfiguration anpassen. Hier ist ein Beispiel für die Angabe der Provider-Routing-Reihenfolge.
...
:::tip
Sie können jeden OpenAI-kompatiblen Anbieter mit KiyoCode verwenden. Die meisten modernen AI-Anbieter bieten OpenAI-kompatible APIs an.
:::
...
Mithilfe der `limit`-Felder kann KiyoCode erkennen, wie viel Kontext Sie noch haben. Standardanbieter beziehen diese automatisch von models.dev.
...
2. Überprüfen Sie bei benutzerdefinierten Anbietern die KiyoCode-Konfiguration und:
   - Stellen Sie sicher, dass der im Befehl `/connect` verwendete Anbieter ID mit dem ID in Ihrer KiyoCode-Konfiguration übereinstimmt.
