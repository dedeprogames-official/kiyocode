---
title: Konfiguration
description: Verwenden Sie die Konfiguration KiyoCode JSON.
---

Sie können KiyoCode mithilfe einer JSON-Konfigurationsdatei konfigurieren.

---

## Format

KiyoCode unterstützt die Formate **JSON** und **JSONC** (JSON mit Kommentaren).
...
Platzieren Sie Ihre globale KiyoCode-Konfiguration in `~/.config/opencode/opencode.json`. Verwenden Sie die globale Konfiguration für benutzerweite Einstellungen wie Themen, Anbieter oder Tastenkombinationen.
...
Wenn KiyoCode gestartet wird, sucht es im aktuellen Verzeichnis nach einer Konfigurationsdatei oder durchsucht das nächstgelegene Git-Verzeichnis.
...
Die Konfigurationsdatei verfügt über ein Schema, das in [**`kiyocode.ai/config.json`**](https://kiyocode.ai/config.json) definiert ist.
...
- `mdns` – mDNS-Diensterkennung aktivieren. Dadurch können andere Geräte im Netzwerk Ihres KiyoCode-Servers erkannt werden.
...
Sie können die Anbieter und Modelle, die Sie in Ihrer KiyoCode-Konfiguration verwenden möchten, über die Optionen `provider`, `model` und `small_model` konfigurieren.
...
Die Option `small_model` konfiguriert ein separates Modell für einfache Aufgaben wie die Titelgenerierung. Standardmäßig versucht KiyoCode, ein günstigeres Modell zu verwenden, sofern eines bei Ihrem Anbieter verfügbar ist, andernfalls greift es auf Ihr Hauptmodell zurück.
...
Sie können das Thema, das Sie in Ihrer KiyoCode-Konfiguration verwenden möchten, über die Option `theme` konfigurieren.
...
Der Standardagent muss ein Primäragent (kein Subagent) sein. Dies kann ein integrierter Agent wie `"build"` oder `"plan"` oder ein von Ihnen definierter [custom agent](/docs/agents) sein. Wenn der angegebene Agent nicht existiert oder ein Subagent ist, greift KiyoCode mit einer Warnung auf `"build"` zurück.
...
KiyoCode lädt beim Start automatisch alle neuen Updates herunter. Sie können dies mit der Option `autoupdate` deaktivieren.
...
KiyoCode erlaubt standardmäßig alle Vorgänge, ohne dass eine ausdrückliche Genehmigung erforderlich ist. Sie können dies mit der Option `permission` ändern.
...
[Plugins](/docs/plugins) erweitert KiyoCode mit benutzerdefinierten Tools, Hooks und Integrationen.
...
Dies ist nützlich, wenn Sie KiyoCode darauf beschränken möchten, nur bestimmte Anbieter zu verwenden, anstatt sie einzeln zu deaktivieren.
