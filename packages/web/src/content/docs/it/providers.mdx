---
title: Provider
description: Usa qualsiasi provider LLM in KiyoCode.
---

import config from "../../../../config.mjs"
export const console = config.console

KiyoCode usa [AI SDK](https://ai-sdk.dev/) e [Models.dev](https://models.dev) per supportare **oltre 75 provider LLM** e anche modelli locali.

Per aggiungere un provider devi:

1. Aggiungere le API key del provider con il comando `/connect`.
2. Configurare il provider nella configurazione di KiyoCode.
...
KiyoCode Zen e una lista di modelli forniti dal team KiyoCode, testati e
verificati per funzionare bene con KiyoCode. [Scopri di piu](/docs/zen).

:::tip
Se sei all'inizio, ti consigliamo di partire da KiyoCode Zen.
:::

1. Esegui `/connect` nella TUI, seleziona kiyocode e vai su [kiyocode.ai/auth](https://kiyocode.ai/auth).
...
Funziona come qualsiasi altro provider in KiyoCode ed e totalmente opzionale.
...
Per usare Amazon Bedrock con KiyoCode:
...
2. **Configura l'autenticazione** usando uno dei seguenti metodi:
   ...
   ***
   ...
   Imposta una di queste variabili d'ambiente mentre esegui kiyocode:
...
:::info
L'uso dell'abbonamento Claude Pro/Max in KiyoCode non è ufficialmente supportato da [Anthropic](https://anthropic.com).
:::
...
Puoi configurare kiyocode per utilizzare modelli locali tramite [Atomic Chat](https://atomic.chat), un'applicazione desktop che esegue LLM locali dietro un server API compatibile OpenAI (endpoint predefinito `http://127.0.0.1:1337/v1`).
...
2. Vai su [Azure AI Foundry](https://ai.azure.com/) e fai il deploy di un modello.
   ...
   :::note
   Il nome del deployment deve corrispondere al nome del modello affinché kiyocode funzioni correttamente.
   :::
...
5. Esegui il comando `/models` per selezionare un modello.
   ...
   Puoi anche aggiungere modelli tramite la tua configurazione di kiyocode.
...
salvare il token nello storage di auth di kiyocode.
:::

##### GitLab Self-Hosted

:::note[nota di compliance]
KiyoCode usa un modello piccolo per alcuni task AI come generare il titolo della sessione.
È configurato per usare gpt-5-nano di default, ospitato da Zen. Per bloccare KiyoCode
...
Per usare il tuo abbonamento GitHub Copilot con kiyocode:
...
Per usare Google Vertex AI con KiyoCode:
...
2. Imposta le variabili d'ambiente richieste:
   ...
   Impostale mentre esegui kiyocode.
...
Nel caso vedessi una funzionalità o un modello di Helicone che non viene configurato automaticamente da kiyocode, puoi sempre configurarlo tu stesso.
...
La funzionalità [Sessions](https://docs.helicone.ai/features/sessions) di Helicone ti permette di raggruppare richieste LLM correlate. Usa il plugin [opencode-helicone-session](https://github.com/H2Shami/opencode-helicone-session) per loggare automaticamente ogni conversazione di KiyoCode come una sessione in Helicone.
...
Il plugin inietta gli header `Helicone-Session-Id` e `Helicone-Session-Name` nelle tue richieste. Nella pagina Sessions di Helicone, vedrai ogni conversazione di KiyoCode elencata come sessione separata.
...
Puoi configurare kiyocode per usare modelli locali tramite l'utility llama-server di [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
Puoi configurare kiyocode per usare modelli locali tramite LM Studio.
...
Puoi configurare kiyocode per usare modelli locali tramite Ollama.

:::tip
Ollama può auto-configurarsi per KiyoCode. Vedi la [documentazione di integrazione Ollama](https://docs.ollama.com/integrations/kiyocode) per dettagli.
:::
...
Per usare Ollama Cloud con KiyoCode:
...
3. Copia la chiave API da usare in KiyoCode.
...
6. **Importante**: Prima di usare modelli cloud in KiyoCode, devi scaricare le informazioni del modello localmente:
...
KiyoCode Zen è una lista di modelli testati e verificati forniti dal team KiyoCode. [Scopri di più](/docs/zen).

1. Accedi a **<a href={console}>KiyoCode Zen</a>** e clicca **Create API Key**.

2. Esegui il comando `/connect` e cerca **KiyoCode Zen**.

   ```txt
   /connect
   ```

3. Inserisci la tua chiave API di KiyoCode.
...
4. Molti modelli OpenRouter sono precaricati di default, esegui il comando `/models` per selezionare quello che vuoi.
   ...
   Puoi anche aggiungere modelli addizionali tramite la tua configurazione di kiyocode.
...
5. Puoi anche personalizzarli tramite la configurazione di kiyocode. Ecco un esempio di specifica di un provider
...
Per usare le [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) con Kiyocode:
...
Puoi anche personalizzare i modelli tramite la tua configurazione di kiyocode. Ecco un esempio di come specificare l'ordine di routing dei provider.
...
:::tip
Puoi usare qualsiasi provider OpenAI-compatible con kiyocode. La maggior parte dei provider AI moderni offre API OpenAI-compatible.
:::
...
I campi `limit` permettono a KiyoCode di capire quanto contesto rimane. I provider standard recuperano questi dati da models.dev automaticamente.
...
2. Per provider personalizzati, controlla la config di kiyocode e:
   - Assicurati che l'ID provider usato nel comando `/connect` corrisponda all'ID nella tua config kiyocode.
