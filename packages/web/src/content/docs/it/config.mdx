...
description: Usa la configurazione JSON di KiyoCode.
---

Puoi configurare KiyoCode usando un file di configurazione JSON.

---

## Formato

KiyoCode supporta sia **JSON** sia **JSONC** (JSON con commenti).
...
Metti la configurazione globale di KiyoCode in `~/.config/opencode/opencode.json`. Usa la configurazione globale per preferenze server/runtime valide per l'utente come provider, modelli e permessi.
...
Quando KiyoCode si avvia, cerca un file di configurazione nella directory corrente oppure risale fino alla directory Git piu vicina.
...
Il file di configurazione ha uno schema definito in [**`kiyocode.ai/config.json`**](https://kiyocode.ai/config.json).

La configurazione TUI usa [**`kiyocode.ai/tui.json`**](https://kiyocode.ai/tui.json).
...
- `mdns` - Abilita la scoperta del servizio mDNS. Questo permette ad altri dispositivi sulla rete di scoprire il tuo server KiyoCode.
...
Puoi configurare provider e modelli da usare in KiyoCode tramite le opzioni `provider`, `model` e `small_model`.
...
L'opzione `small_model` configura un modello separato per task leggeri come la generazione dei titoli. Per impostazione predefinita, KiyoCode prova a usare un modello più economico se disponibile dal tuo provider, altrimenti fa fallback sul tuo modello principale.
...
L'agente predefinito deve essere un agente primary (non un subagent). Puo essere un agente integrato come `"build"` o `"plan"`, oppure un [agente personalizzato](/docs/agents) che hai definito. Se l'agente specificato non esiste o e un subagent, KiyoCode fara fallback su `"build"` mostrando un avviso.
...
KiyoCode scarichera automaticamente eventuali aggiornamenti quando si avvia. Puoi disabilitare questa funzione con l'opzione `autoupdate`.
...
Per impostazione predefinita, kiyocode **consente tutte le operazioni** senza richiedere un'approvazione esplicita. Puoi cambiarlo usando l'opzione `permission`.
...
[I plugin](/docs/plugins) estendono KiyoCode con strumenti personalizzati, hook e integrazioni.
...
E utile quando vuoi limitare KiyoCode a usare solo alcuni provider invece di disabilitarli uno per uno.
