---
title: IDE
description: L'estensione KiyoCode per VS Code, Cursor e altri IDE
---

KiyoCode si integra con VS Code, Cursor o qualunque IDE che supporti un terminale. Basta eseguire `opencode` nel terminale per iniziare.

---

## Utilizzo

- **Avvio rapido**: usa `Cmd+Esc` (Mac) o `Ctrl+Esc` (Windows/Linux) per aprire KiyoCode in una vista terminale divisa, oppure per mettere a fuoco una sessione esistente se e gia in esecuzione.
- **Nuova sessione**: usa `Cmd+Shift+Esc` (Mac) o `Ctrl+Shift+Esc` (Windows/Linux) per avviare una nuova sessione di KiyoCode, anche se ne esiste gia una aperta. Puoi anche cliccare il pulsante KiyoCode nell'interfaccia.
- **Consapevolezza del contesto**: condividi automaticamente con KiyoCode la selezione corrente o la scheda attiva.
...
Per installare KiyoCode su VS Code e fork popolari come Cursor, Windsurf, VSCodium:
...
Cerca **KiyoCode** nel Marketplace delle estensioni e clicca **Install**.
