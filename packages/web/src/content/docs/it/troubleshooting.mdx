...
Per diagnosticare problemi con KiyoCode, inizia controllando i log e i dati locali che salva su disco.
...
kiyocode salva i dati delle sessioni e altri dati dell'applicazione su disco in:
...
KiyoCode Desktop esegue in background un server locale di KiyoCode (il sidecar `opencode-cli`). La maggior parte dei problemi e causata da un plugin che si comporta male, da una cache corrotta o da un'impostazione del server errata.
...
KiyoCode puo anche caricare plugin locali dal disco. Spostali temporaneamente altrove (o rinomina la cartella) e riavvia l'app desktop:
...
Se disabilitare i plugin non aiuta (o l'installazione di un plugin e bloccata), svuota la cache in modo che KiyoCode possa ricostruirla.

1. Chiudi completamente KiyoCode Desktop.
...
3. Riavvia KiyoCode Desktop.

---

### Risolvi problemi di connessione al server

KiyoCode Desktop puo avviare il proprio server locale (predefinito) oppure connettersi a un URL server che hai configurato.
...
Su Windows, KiyoCode Desktop richiede **WebView2 Runtime** di Microsoft Edge. Se l'app si apre su una finestra vuota o non parte, installa/aggiorna WebView2 e riprova.

---

### Windows: problemi generali di prestazioni

Se riscontri prestazioni lente, problemi di accesso ai file o problemi del terminale su Windows, prova a usare [WSL (Windows Subsystem for Linux)](/docs/windows-wsl). WSL fornisce un ambiente Linux che funziona in modo piu fluido con le funzionalita di KiyoCode.

---

### Notifiche non visualizzate

KiyoCode Desktop mostra le notifiche di sistema solo quando:

- le notifiche sono abilitate per KiyoCode nelle impostazioni del sistema operativo, e
...
1. Chiudi KiyoCode Desktop.
2. Trova ed elimina questi file (si trovano nella directory dati dell'app KiyoCode Desktop):
...
Se riscontri problemi con KiyoCode:
...
Se incontri errori nelle chiamate API, potrebbe dipendere da pacchetti provider non aggiornati. kiyocode installa dinamicamente i pacchetti provider (OpenAI, Anthropic, Google, ecc.) quando servono e li mette in cache localmente.
...
2. Riavvia kiyocode per reinstallare i pacchetti provider piu recenti

Questo forzera kiyocode a scaricare le versioni piu recenti dei pacchetti provider, cosa che spesso risolve problemi di compatibilita con parametri dei modelli e cambiamenti delle API.
...
kiyocode rilevera se stai usando Wayland e preferira `wl-clipboard`; altrimenti provera a trovare gli strumenti per gli appunti nell'ordine: `xclip` e `xsel`.
