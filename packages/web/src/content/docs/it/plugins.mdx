---
title: Plugin
description: Scrivi plugin per estendere KiyoCode.
---

I plugin ti permettono di estendere KiyoCode agganciandoti a vari eventi e personalizzando il comportamento. Puoi creare plugin per aggiungere nuove funzionalità, integrare servizi esterni o modificare il comportamento predefinito di KiyoCode.
...
KiyoCode esegue `bun install` all'avvio per installarle. I tuoi plugin e strumenti potranno poi importarle.
...
- `client`: un client SDK di kiyocode per interagire con l'AI.
...
Ecco alcuni esempi di plugin che puoi usare per estendere kiyocode.
...
:::note
Se usi l'app desktop di KiyoCode, può inviare automaticamente notifiche di sistema quando una risposta è pronta o quando una sessione va in errore.
:::
...
Impedisci a kiyocode di leggere i file `.env`:
...
I plugin possono anche aggiungere strumenti personalizzati a kiyocode:
...
L'helper `tool` crea uno strumento personalizzato che kiyocode può chiamare. Accetta una funzione di schema Zod e restituisce una definizione di tool con:
...
I tuoi strumenti personalizzati saranno disponibili in kiyocode insieme agli strumenti integrati.
...
Livelli: `debug`, `info`, `warn`, `error`. Vedi la [documentazione SDK](https://kiyocode.ai/docs/sdk) per i dettagli.
