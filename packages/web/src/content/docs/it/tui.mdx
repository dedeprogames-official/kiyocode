---
title: TUI
description: Usa l'interfaccia utente da terminale (TUI) di KiyoCode.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

KiyoCode fornisce un'interfaccia interattiva da terminale (TUI) per lavorare sui tuoi progetti con un LLM.

Eseguire KiyoCode avvia la TUI per la directory corrente.
...
Quando usi la TUI di KiyoCode, puoi digitare `/` seguito dal nome di un comando per eseguire rapidamente azioni. Per esempio:
...
Aggiunge un provider a KiyoCode. Ti permette di selezionare tra i provider disponibili e aggiungere le relative chiavi API.
...
Esce da KiyoCode. _Alias_: `/quit`, `/q`
...
- `leader_timeout` - Controlla per quanto tempo KiyoCode attende dopo il leader key. Default: `2000`.
