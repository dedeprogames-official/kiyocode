---
title: Server
description: Interagisci con il server kiyocode via HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Il comando `opencode serve` avvia un server HTTP headless che espone un endpoint OpenAPI utilizzabile da un client kiyocode.
...
:::tip
Usa il server kiyocode per interagire con kiyocode in modo programmatico.
:::

Questa architettura permette a kiyocode di supportare piu client e di essere usato in modo programmatico.

Puoi eseguire `opencode serve` per avviare un server standalone. Se la TUI di kiyocode e gia in esecuzione, `opencode serve` avviera un nuovo server.
...
L'endpoint [`/tui`](#tui) puo essere usato per pilotare la TUI tramite il server. Per esempio, puoi precompilare o eseguire un prompt. Questa configurazione e usata dai plugin [IDE](/docs/ide) di KiyoCode.
...
Il server kiyocode espone le seguenti API.
