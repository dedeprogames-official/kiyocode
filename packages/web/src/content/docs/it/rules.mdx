---
title: Regole
description: Imposta istruzioni personalizzate per kiyocode.
---

Puoi fornire istruzioni personalizzate a kiyocode creando un file `AGENTS.md`. E' simile alle regole di Cursor. Contiene istruzioni che verranno incluse nel contesto dell'LLM per personalizzarne il comportamento per il tuo progetto.

---

## Inizializza

Per creare un nuovo file `AGENTS.md`, puoi eseguire il comando `/init` in kiyocode.

:::tip
Dovresti committare il file `AGENTS.md` del progetto in Git.
:::

Questo scansiona il progetto e tutti i suoi contenuti per capire di cosa si tratta e generare un file `AGENTS.md`. Questo aiuta kiyocode a orientarsi meglio nel progetto.
...
kiyocode supporta anche la lettura del file `AGENTS.md` da piu' posizioni, e questo serve a scopi diversi.
...
Puoi anche avere regole globali in un file `~/.config/opencode/AGENTS.md`. Queste si applicano a tutte le sessioni di kiyocode.
...
Per gli utenti che migrano da Claude Code, KiyoCode supporta come fallback le convenzioni di file di Claude Code:
...
Quando kiyocode si avvia, cerca i file di regole in questo ordine:
...
Anche se kiyocode non interpreta automaticamente i riferimenti a file in `AGENTS.md`, puoi ottenere una funzionalita' simile in due modi:
...
Puoi insegnare a kiyocode a leggere file esterni fornendo istruzioni esplicite nel tuo `AGENTS.md`. Ecco un esempio pratico:
...
- Assicurati che kiyocode carichi file solo quando servono per il task specifico
