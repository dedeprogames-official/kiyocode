---
title: CLI
description: Opzioni e comandi della CLI di KiyoCode.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

La CLI di KiyoCode, per impostazione predefinita, avvia la [TUI](/docs/tui) quando viene eseguita senza argomenti.

```bash
opencode
```

Ma accetta anche i comandi documentati in questa pagina. Questo ti permette di interagire con KiyoCode in modo programmatico.
...
Avvia l'interfaccia testuale di KiyoCode (interfaccia utente da terminale).
...
La CLI di KiyoCode include anche i seguenti comandi.

---

### agent

Gestisci gli agenti per KiyoCode.
...
Collega un terminale a un backend server di KiyoCode già in esecuzione avviato tramite i comandi `serve` o `web`.

```bash
opencode attach [url]
```

Questo consente di usare la TUI con un backend KiyoCode remoto. Per esempio:
...
KiyoCode si basa sull'elenco provider di [Models.dev](https://models.dev), quindi puoi usare `opencode auth login` per configurare le API key per qualunque provider tu voglia usare. Le credenziali vengono salvate in `~/.local/share/opencode/auth.json`.

```bash
opencode auth login
```

Quando KiyoCode si avvia, carica i provider dal file delle credenziali e, se presenti, anche eventuali key definite nell'ambiente o in un file `.env` nel progetto.
...
Usa `--refresh` per aggiornare l'elenco modelli in cache. È utile quando nuovi modelli vengono aggiunti a un provider e vuoi vederli in KiyoCode.
...
Esegui kiyocode in modalità non interattiva passando un prompt direttamente.
...
| <nobr><code>{"--attach"}</code></nobr>   |       | Attach a un server kiyocode in esecuzione (es. http://localhost:4096)                           |
| <nobr><code>{"--password"}</code></nobr> | `-p`  | Password per l'autenticazione di base (predefinita: `OPENCODE_SERVER_PASSWORD`)                 |
| <nobr><code>{"--username"}</code></nobr> | `-u`  | Nome utente per l'autenticazione di base (predefinito: `OPENCODE_SERVER_USERNAME` o `opencode`) |
...
Avvia un server KiyoCode headless per accesso via API. Vedi le [server docs](/docs/server) per l'interfaccia HTTP completa.

```bash
opencode serve
```

Avvia un server HTTP che espone accesso API alle funzionalità di kiyocode senza la TUI. Imposta `OPENCODE_SERVER_PASSWORD` per abilitare HTTP basic auth (username di default `opencode`).
...
Elenca tutte le sessioni KiyoCode.
...
Mostra statistiche di utilizzo token e costo per le sessioni KiyoCode.
...
Importa i dati di sessione da un file JSON o da un URL di condivisione KiyoCode.

```bash
opencode import <file>
```

Puoi importare da un file locale o da un URL di condivisione KiyoCode.
...
Avvia un server KiyoCode headless con interfaccia web.

```bash
opencode web
```

Avvia un server HTTP e apre un browser per accedere a KiyoCode tramite interfaccia web. Imposta `OPENCODE_SERVER_PASSWORD` per abilitare HTTP basic auth (username di default `opencode`).
...
Disinstalla KiyoCode e rimuove tutti i file correlati.
...
Aggiorna kiyocode all'ultima versione o a una versione specifica.
...
La CLI di kiyocode accetta i seguenti flag globali.
...
KiyoCode può essere configurato tramite variabili d'ambiente.
