...
In KiyoCode esistono due tipi di agenti: agenti primari e subagenti.
...
KiyoCode include due agenti primari integrati: **Build** e **Plan**. Li vediamo sotto.
...
KiyoCode include tre subagenti integrati: **General**, **Explore** e **Scout**. Li vediamo sotto.

---

## Integrati

KiyoCode include due agenti primari integrati e tre subagenti integrati.
...
Un agente in sola lettura per la ricerca su documentazione esterna e dipendenze. Usalo quando devi clonare il repository di una dipendenza nella cache gestita di KiyoCode, ispezionare il codice sorgente di una libreria o confrontare il codice locale con implementazioni upstream senza modificare il tuo workspace.
...
Se non specifichi una temperature, KiyoCode usa i default specifici del modello; tipicamente 0 per la maggior parte dei modelli e 0.55 per i modelli Qwen.
...
Questo path è relativo alla posizione del file di configurazione. Quindi funziona sia per la config globale di KiyoCode sia per la config specifica del progetto.
...
L'ID modello nella configurazione KiyoCode usa il formato `provider/model-id`. Per esempio, se stai usando [KiyoCode Zen](/docs/zen), useresti `opencode/gpt-5.1-codex` per GPT 5.1 Codex.
...
:::tip
Hai un agente che vorresti condividere? [Invia una PR](https://github.com/dedeprogames-official/kiyocode).
:::
