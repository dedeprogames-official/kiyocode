---
title: Configuração
description: Usando a configuração JSON do kiyocode.
---

Você pode configurar o kiyocode usando um arquivo de configuração JSON.

---

## Formato

O kiyocode suporta os formatos **JSON** e **JSONC** (JSON com Comentários).
...
Coloque sua configuração global do kiyocode em `~/.config/opencode/opencode.json`. Use a configuração global para preferências de todo o usuário, como provedores, modelos e permissões.
...
Quando o kiyocode é iniciado, ele procura um arquivo de configuração no diretório atual ou sobe até o diretório Git mais próximo.
...
O esquema de configuração do servidor/tempo de execução é definido em [**`kiyocode.ai/config.json`**](https://kiyocode.ai/config.json).

A configuração do TUI usa [**`kiyocode.ai/tui.json`**](https://kiyocode.ai/tui.json).
...
- `mdns` - Habilitar descoberta de serviço mDNS. Isso permite que outros dispositivos na rede descubram seu servidor kiyocode.
...
Você pode configurar os provedores e modelos que deseja usar em sua configuração do kiyocode através das opções `provider`, `model` e `small_model`.
...
A opção `small_model` configura um modelo separado para tarefas leves, como geração de títulos. Por padrão, o kiyocode tenta usar um modelo mais barato se um estiver disponível do seu provedor, caso contrário, ele recua para seu modelo principal.
...
O agente padrão deve ser um agente primário (não um subagente). Isso pode ser um agente embutido como `"build"` ou `"plan"`, ou um [agente personalizado](/docs/agents) que você definiu. Se o agente especificado não existir ou for um subagente, o kiyocode recuará para `"build"` com um aviso.
...
O kiyocode fará o download automaticamente de quaisquer novas atualizações quando for iniciado. Você pode desabilitar isso com a opção `autoupdate`.
...
Por padrão, o kiyocode **permite todas as operações** sem exigir aprovação explícita. Você pode mudar isso usando a opção `permission`.
...
[Plugins](/docs/plugins) estendem o kiyocode com ferramentas, hooks e integrações personalizadas.
...
Isso é útil quando você deseja restringir o kiyocode para usar apenas provedores específicos, em vez de desabilitá-los um a um.
