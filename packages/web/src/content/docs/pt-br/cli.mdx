---
title: CLI
description: Opções e comandos da CLI do kiyocode.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

A CLI do kiyocode, por padrão, inicia o [TUI](/docs/tui) quando executada sem argumentos.

```bash
opencode
```

Mas também aceita comandos conforme documentado nesta página. Isso permite que você interaja com o kiyocode programaticamente.
...
Inicie a interface de usuário do terminal do kiyocode.
...
A CLI do kiyocode também possui os seguintes comandos.

---

### agent

Gerencie agentes para o kiyocode.
...
Anexe um terminal a um servidor backend do kiyocode já em execução, iniciado via comandos `serve` ou `web`.

```bash
opencode attach [url]
```

Isso permite usar o TUI com um backend kiyocode remoto. Por exemplo:
...
O kiyocode é alimentado pela lista de provedores em [Models.dev](https://models.dev), então você pode usar `opencode auth login` para configurar chaves de API para qualquer provedor que você gostaria de usar. Isso é armazenado em `~/.local/share/opencode/auth.json`.

```bash
opencode auth login
```

Quando o kiyocode é iniciado, ele carrega os provedores do arquivo de credenciais. E se houver chaves definidas em seus ambientes ou em um arquivo `.env` em seu projeto.
...
Use a flag `--refresh` para atualizar a lista de modelos em cache. Isso é útil quando novos modelos foram adicionados a um provedor e você deseja vê-los no kiyocode.
...
Execute o kiyocode em modo não interativo passando um prompt diretamente.
...
| <nobr><code>{"--attach"}</code></nobr>   |       | Anexe a um servidor kiyocode em execução (por exemplo, http://localhost:4096)     |
| <nobr><code>{"--password"}</code></nobr> | `-p`  | Senha de autenticação básica (padrão: `OPENCODE_SERVER_PASSWORD`)                 |
| <nobr><code>{"--username"}</code></nobr> | `-u`  | Usuário de autenticação básica (padrão: `OPENCODE_SERVER_USERNAME` ou `opencode`) |
...
Inicie um servidor kiyocode sem cabeça para acesso à API. Confira a [documentação do servidor](/docs/server) para a interface HTTP completa.

```bash
opencode serve
```

Isso inicia um servidor HTTP que fornece acesso à funcionalidade do kiyocode sem a interface TUI. Defina `OPENCODE_SERVER_PASSWORD` para habilitar a autenticação básica HTTP (o nome de usuário padrão é `opencode`).
...
Liste todas as sessões do kiyocode.
...
Mostre o uso de tokens e estatísticas de custo para suas sessões do kiyocode.
...
Importe dados da sessão de um arquivo JSON ou URL de compartilhamento do kiyocode.

```bash
opencode import <file>
```

Você pode importar de um arquivo local ou de uma URL de compartilhamento do kiyocode.
...
Inicie um servidor kiyocode sem cabeça com uma interface web.

```bash
opencode web
```

Isso inicia um servidor HTTP e abre um navegador para acessar o kiyocode através de uma interface web. Defina `OPENCODE_SERVER_PASSWORD` para habilitar a autenticação básica HTTP (o nome de usuário padrão é `opencode`).
...
Desinstale o kiyocode e remova todos os arquivos relacionados.
...
Atualiza o kiyocode para a versão mais recente ou uma versão específica.
...
A CLI do kiyocode aceita as seguintes flags globais.
...
O kiyocode pode ser configurado usando variáveis de ambiente.
