---
title: Provedores
description: Usando qualquer provedor LLM no kiyocode.
---

import config from "../../../../config.mjs"
export const console = config.console

kiyocode usa o [AI SDK](https://ai-sdk.dev/) e [Models.dev](https://models.dev) para suportar **75+ provedores LLM** e suporta a execução de modelos locais.

Para adicionar um provedor, você precisa:

1. Adicionar as chaves da API para o provedor usando o comando `/connect`.
2. Configurar o provedor na sua configuração do kiyocode.
...
Você pode personalizar os provedores através da seção `provider` na sua configuração do kiyocode.
...
KiyoCode Zen é uma lista de modelos fornecidos pela equipe do kiyocode que foram testados e verificados para funcionar bem com o kiyocode. [Saiba mais](/docs/zen).

:::tip
Se você é novo, recomendamos começar com o KiyoCode Zen.
:::

1. Execute o comando `/connect` no TUI, selecione `OpenCode Zen` e acesse [kiyocode.ai/zen](https://kiyocode.ai/zen).
...
Funciona como qualquer outro provedor no kiyocode e é completamente opcional.

---

## KiyoCode Go

KiyoCode Go é um plano de assinatura de baixo custo que fornece acesso confiável a modelos de codificação abertos populares fornecidos pela equipe do kiyocode que foram
testados e verificados para funcionar bem com o kiyocode.

1. Execute o comando `/connect` no TUI, selecione `OpenCode Go` e acesse [kiyocode.ai/zen](https://kiyocode.ai/zen).
...
Para usar o Amazon Bedrock com o kiyocode:
...
2. **Configure a autenticação** usando um dos seguintes métodos:
   ...
   ***
   ...
   Defina uma dessas variáveis de ambiente ao executar o kiyocode:
...
:::info
Usar sua assinatura Claude Pro/Max no kiyocode não é oficialmente suportado pela [Anthropic](https://anthropic.com).
:::
...
Você pode configurar o kiyocode para usar modelos locais através do [Atomic Chat](https://atomic.chat), um aplicativo de desktop que executa LLMs locais por trás de um servidor API compatível com OpenAI (endpoint padrão `http://127.0.0.1:1337/v1`).
...
2. Vá para [Azure AI Foundry](https://ai.azure.com/) e implante um modelo.
   ...
   :::note
   O nome da implantação deve corresponder ao nome do modelo para que o kiyocode funcione corretamente.
   :::
...
5. Execute o comando `/models` para selecionar um modelo.
   ...
   Você também pode adicionar modelos através da sua configuração do kiyocode.
...
armazenar o token no armazenamento de autenticação do kiyocode.
:::

##### GitLab Auto-Hospedado

:::note[nota de conformidade]
kiyocode usa um pequeno modelo para algumas tarefas de AI, como gerar o título da sessão.
Ele está configurado para usar gpt-5-nano por padrão, hospedado pela Zen. Para bloquear o kiyocode
...
Para usar sua assinatura do GitHub Copilot com o kiyocode:
...
Para usar o Google Vertex AI com o kiyocode:
...
2. Defina as variáveis de ambiente necessárias:
   ...
   Defina-as ao executar o kiyocode.
...
Caso você veja um recurso ou modelo do Helicone que não está configurado automaticamente através do kiyocode, você sempre pode configurá-lo você mesmo.
...
O recurso [Sessões](https://docs.helicone.ai/features/sessions) do Helicone permite que você agrupe solicitações LLM relacionadas. Use o plugin [opencode-helicone-session](https://github.com/H2Shami/opencode-helicone-session) para registrar automaticamente cada conversa do kiyocode como uma sessão no Helicone.
...
O plugin injeta os cabeçalhos `Helicone-Session-Id` e `Helicone-Session-Name` em suas solicitações. Na página de Sessões do Helicone, você verá cada conversa do kiyocode listada como uma sessão separada.
...
Você pode configurar o kiyocode para usar modelos locais através da utilidade llama-server do [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
Você pode configurar o kiyocode para usar modelos locais através do LM Studio.
...
Você pode configurar o kiyocode para usar modelos locais através do Ollama.

:::tip
Ollama pode se configurar automaticamente para o kiyocode. Consulte a [documentação de integração do Ollama](https://docs.ollama.com/integrations/kiyocode) para detalhes.
:::
...
Para usar o Ollama Cloud com o kiyocode:
...
3. Copie a chave da API para uso no kiyocode.
...
6. **Importante**: Antes de usar modelos em nuvem no kiyocode, você deve puxar as informações do modelo localmente:
...
KiyoCode Zen é uma lista de modelos testados e verificados fornecidos pela equipe do kiyocode. [Saiba mais](/docs/zen).

1. Faça login em **<a href={console}>KiyoCode Zen</a>** e clique em **Criar Chave da API**.

2. Execute o comando `/connect` e procure por **KiyoCode Zen**.

   ```txt
   /connect
   ```

3. Insira sua chave da API do kiyocode.
...
4. Muitos modelos do OpenRouter são pré-carregados por padrão, execute o comando `/models` para selecionar o que você deseja.
   ...
   Você também pode adicionar modelos adicionais através da sua configuração do kiyocode.
...
5. Você também pode personalizá-los através da sua configuração do kiyocode. Aqui está um exemplo de especificar um provedor
...
Para usar [APIs Generativas Scaleway](https://www.scaleway.com/en/docs/generative-apis/) com o kiyocode:
...
Você também pode personalizar modelos através da sua configuração do kiyocode. Aqui está um exemplo de especificar a ordem de roteamento do provedor.
...
:::tip
Você pode usar qualquer provedor compatível com OpenAI com o kiyocode. A maioria dos provedores de AI modernos oferece APIs compatíveis com OpenAI.
:::
...
Os campos `limit` permitem que o kiyocode entenda quanto contexto você tem disponível. Provedores padrão puxam isso automaticamente do models.dev.
...
2. Para provedores personalizados, verifique a configuração do kiyocode e:
   - Certifique-se de que o ID do provedor usado no comando `/connect` corresponda ao ID na sua configuração do kiyocode.
