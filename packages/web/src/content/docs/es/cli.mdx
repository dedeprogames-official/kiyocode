---
title: CLI
description: Opciones y comandos de la CLI de KiyoCode.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

La CLI de KiyoCode de forma predeterminada inicia el [TUI](/docs/tui) cuando se ejecuta sin ningún argumento.

```bash
opencode
```

Pero también acepta comandos como se documenta en esta página. Esto le permite interactuar con KiyoCode mediante programación.
...
Inicie la interfaz de usuario del terminal KiyoCode.
...
La CLI de KiyoCode también tiene los siguientes comandos.

---

### agent

Administrar agentes para KiyoCode.
...
Conecte una terminal a un servidor backend KiyoCode que ya se esté ejecutando y iniciado mediante los comandos `serve` o `web`.

```bash
opencode attach [url]
```

Esto permite usar TUI con un backend remoto KiyoCode. Por ejemplo:
...
KiyoCode funciona con la lista de proveedores en [Models.dev](https://models.dev), por lo que puede usar `opencode auth login` para configurar las claves API para cualquier proveedor que desee utilizar. Esto se almacena en `~/.local/share/opencode/auth.json`.

```bash
opencode auth login
```

Cuando se inicia KiyoCode, carga los proveedores desde el archivo de credenciales. Y si hay claves definidas en sus entornos o un archivo `.env` en su proyecto.
...
Utilice el indicador `--refresh` para actualizar la lista de modelos almacenados en caché. Esto es útil cuando se han agregado nuevos modelos a un proveedor y desea verlos en KiyoCode.
...
Ejecute kiyocode en modo no interactivo pasando un mensaje directamente.
...
| <nobr><code>{"--attach"}</code></nobr>   |       | Adjuntar a un servidor kiyocode en ejecución (por ejemplo, http://localhost:4096)         |
| <nobr><code>{"--password"}</code></nobr> | `-p`  | Contraseña de autenticación básica (predeterminada: `OPENCODE_SERVER_PASSWORD`)           |
| <nobr><code>{"--username"}</code></nobr> | `-u`  | Usuario de autenticación básica (predeterminado: `OPENCODE_SERVER_USERNAME` u `opencode`) |
...
Inicie un servidor KiyoCode sin cabeza para acceso API. Consulte los [documentos del servidor](/docs/server) para conocer la interfaz HTTP completa.

```bash
opencode serve
```

Esto inicia un servidor HTTP que proporciona acceso API a la funcionalidad kiyocode sin la interfaz TUI. Configure `OPENCODE_SERVER_PASSWORD` para habilitar la autenticación básica HTTP (el nombre de usuario predeterminado es `opencode`).
...
Enumere todas las sesiones KiyoCode.
...
Muestre el uso de tokens y las estadísticas de costos para sus sesiones KiyoCode.
...
Importe datos de sesión desde un archivo JSON o una URL compartida KiyoCode.

```bash
opencode import <file>
```

Puede importar desde un archivo local o una URL compartida KiyoCode.
...
Inicie un servidor KiyoCode sin cabeza con una interfaz web.

```bash
opencode web
```

Esto inicia un servidor HTTP y abre un navegador web para acceder a KiyoCode a través de una interfaz web. Configure `OPENCODE_SERVER_PASSWORD` para habilitar la autenticación básica HTTP (el nombre de usuario predeterminado es `opencode`).
...
Desinstale KiyoCode y elimine todos los archivos relacionados.
...
Actualiza kiyocode a la última versión o a una versión específica.
...
La CLI de KiyoCode toma las siguientes banderas globales.
...
KiyoCode se puede configurar mediante variables de entorno.
