...
Los proveedores más populares están precargados de forma predeterminada. Si agregó las credenciales de un proveedor mediante el comando `/connect`, estarán disponibles cuando inicie KiyoCode.
...
Aqui tienes varios modelos que funcionan bien con KiyoCode, sin orden particular. (Esta no es una lista exhaustiva ni necesariamente actualizada):
...
Aquí el ID completo es `provider_id/model_id`. Por ejemplo, si usa [KiyoCode Zen](/docs/zen), usaría `opencode/gpt-5.1-codex` para GPT 5.1 Codex.
...
Muchos modelos admiten múltiples variantes con diferentes configuraciones. KiyoCode se envía con variantes predeterminadas integradas para proveedores populares.

### Variantes integradas

KiyoCode se envía con variantes predeterminadas para muchos proveedores:
...
Cuando se inicia KiyoCode, busca modelos en el siguiente orden de prioridad:
...
2. La lista de modelos en la configuración KiyoCode.
