---
title: Proveedores
description: Usando cualquier proveedor LLM en KiyoCode.
---

import config from "../../../../config.mjs"
export const console = config.console

KiyoCode usa el [AI SDK](https://ai-sdk.dev/) y [Models.dev](https://models.dev) para admitir **75+ proveedores LLM** y admite la ejecución de modelos locales.

Para agregar un proveedor necesita:

1. Agregue las claves API para el proveedor usando el comando `/connect`.
2. Configure el proveedor en su configuración KiyoCode.
...
Puedes personalizar los proveedores a través de la sección `provider` en tu KiyoCode
configuración.
...
KiyoCode Zen es una lista de modelos proporcionados por el equipo KiyoCode que han sido
probado y verificado para funcionar bien con KiyoCode. [Más información](/docs/zen).

:::tip
Si eres nuevo, te recomendamos comenzar con KiyoCode Zen.
:::

1. Ejecute el comando `/connect` en TUI, seleccione kiyocode y diríjase a [kiyocode.ai/auth](https://kiyocode.ai/auth).
...
Funciona como cualquier otro proveedor en KiyoCode y su uso es completamente opcional.

---

## KiyoCode Go

KiyoCode Go es un plan de suscripción de bajo costo que brinda acceso confiable a modelos de codificación abiertos populares proporcionados por el equipo de KiyoCode que han sido
probado y verificado para funcionar bien con KiyoCode.

1. Ejecute el comando `/connect` en TUI, seleccione `OpenCode Go` y diríjase a [kiyocode.ai/auth](https://kiyocode.ai/zen).
...
Para usar Amazon Bedrock con KiyoCode:
...
2. **Configure la autenticación** utilizando uno de los siguientes métodos:
   ...
   Establezca una de estas variables de entorno mientras ejecuta kiyocode:
...
:::info
El uso de su suscripción Claude Pro/Max en KiyoCode no está oficialmente respaldado por [Anthropic](https://anthropic.com).
:::
...
Puedes configurar kiyocode para usar modelos locales mediante [Atomic Chat](https://atomic.chat), una aplicación de escritorio que ejecuta LLMs locales detrás de un servidor API compatible con OpenAI (endpoint por defecto `http://127.0.0.1:1337/v1`).
...
2. Vaya a [Azure AI Foundry](https://ai.azure.com/) e implemente un modelo.
   ...
   :::nota
   El nombre de la implementación debe coincidir con el nombre del modelo para que kiyocode funcione correctamente.
   :::
...
5. Ejecute el comando `/models` para seleccionar un modelo.
   ...
   También puede agregar modelos a través de su configuración kiyocode.
...
para almacenar el token en el almacenamiento de autenticación kiyocode.
:::

##### Autohospedado GitLab

:::note[nota de cumplimiento]
KiyoCode utiliza un modelo pequeño para algunas tareas de IA, como generar el título de la sesión.
Está configurado para usar gpt-5-nano de forma predeterminada, alojado por Zen. Para bloquear KiyoCode
...
Para utilizar su suscripción GitHub Copilot con kiyocode:
...
Para utilizar Google Vertex AI con KiyoCode:
...
2. Establezca las variables de entorno requeridas:
   ...
   Configúrelos mientras ejecuta kiyocode.
...
En caso de que vea una característica o modelo de Helicone que no esté configurado automáticamente a través de kiyocode, siempre podrá configurarlo usted mismo.
...
La función [Sesiones](https://docs.helicone.ai/features/sessions) de Helicone le permite agrupar solicitudes LLM relacionadas. Utilice el complemento [opencode-helicone-session](https://github.com/H2Shami/opencode-helicone-session) para registrar automáticamente cada conversación KiyoCode como una sesión en Helicone.
...
El complemento inyecta encabezados `Helicone-Session-Id` y `Helicone-Session-Name` en sus solicitudes. En la página Sesiones de Helicone, verá cada conversación KiyoCode enumerada como una sesión separada.
...
Puede configurar kiyocode para usar modelos locales a través de la utilidad llama-server de [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
Puede configurar kiyocode para usar modelos locales a través de LM Studio.
...
Puedes configurar kiyocode para usar modelos locales a través de Ollama.

:::tip
Ollama puede configurarse automáticamente para KiyoCode. Consulte los [documentos de integración de Ollama](https://docs.ollama.com/integrations/kiyocode) para obtener más detalles.
:::
...
Para usar Ollama Cloud con KiyoCode:
...
3. Copie la clave API para usarla en KiyoCode.
...
6. **Importante**: Antes de usar modelos en la nube en KiyoCode, debe obtener la información del modelo localmente:
...
KiyoCode Zen es una lista de modelos probados y verificados proporcionada por el equipo KiyoCode. [Más información](/docs/zen).

1. Inicie sesión en **<a href={console}>KiyoCode Zen</a>** y haga clic en **Crear API clave**.

2. Ejecute el comando `/connect` y busque **KiyoCode Zen**.

   ```txt
   /connect
   ```

3. Ingrese su clave KiyoCode API.
...
4. Muchos modelos de OpenRouter están precargados de forma predeterminada, ejecute el comando `/models` para seleccionar el que desee.
   ...
   También puede agregar modelos adicionales a través de su configuración kiyocode.
...
5. También puedes personalizarlos a través de tu configuración kiyocode. A continuación se muestra un ejemplo de cómo especificar un proveedor.
...
Para utilizar [API generativas de Scaleway](https://www.scaleway.com/en/docs/generative-apis/) con KiyoCode:
...
También puede personalizar modelos a través de su configuración kiyocode. A continuación se muestra un ejemplo de cómo especificar el orden de enrutamiento del proveedor.
...
:::tip
Puede utilizar cualquier proveedor compatible con OpenAI con kiyocode. La mayoría de los proveedores de IA modernos ofrecen API compatibles con OpenAI.
:::
...
Los campos `limit` le permiten a KiyoCode comprender cuánto contexto le queda. Los proveedores estándar los extraen de models.dev automáticamente.
...
2. Para proveedores personalizados, verifique la configuración kiyocode y:
   - Asegúrese de que el ID del proveedor utilizado en el comando `/connect` coincida con el ID en su configuración kiyocode.
