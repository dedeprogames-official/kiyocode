---
title: Configuración
description: Usando la configuración KiyoCode JSON.
---

Puede configurar KiyoCode usando un archivo de configuración JSON.

---

## Formato

KiyoCode admite los formatos **JSON** y **JSONC** (JSON con comentarios).
...
Coloque su configuración global KiyoCode en `~/.config/opencode/opencode.json`. Utilice la configuración global para las preferencias de todo el usuario, como temas, proveedores o combinaciones de teclas.
...
Cuando se inicia KiyoCode, busca un archivo de configuración en el directorio actual o recorre hasta el directorio Git más cercano.
...
El archivo de configuración tiene un esquema definido en [**`kiyocode.ai/config.json`**](https://kiyocode.ai/config.json).
...
- `mdns`: habilita el descubrimiento de servicios mDNS. Esto permite que otros dispositivos en la red descubran su servidor KiyoCode.
...
Puede configurar los proveedores y modelos que desea utilizar en su configuración KiyoCode a través de las opciones `provider`, `model` y `small_model`.
...
La opción `small_model` configura un modelo separado para tareas livianas como la generación de títulos. De forma predeterminada, KiyoCode intenta utilizar un modelo más económico si su proveedor tiene uno disponible; de ​​lo contrario, recurre a su modelo principal.
...
Puede configurar el tema que desea usar en su configuración KiyoCode a través de la opción `theme`.
...
El agente predeterminado debe ser un agente principal (no un subagente). Puede ser un agente integrado como `"build"` o `"plan"`, o un [agente personalizado](/docs/agents) que haya definido. Si el agente especificado no existe o es un subagente, KiyoCode recurrirá a `"build"` con una advertencia.
...
KiyoCode descargará automáticamente cualquier actualización nueva cuando se inicie. Puede desactivar esto con la opción `autoupdate`.
...
De forma predeterminada, kiyocode **permite todas las operaciones** sin requerir aprobación explícita. Puede cambiar esto usando la opción `permission`.
...
Los [complementos](/docs/plugins) amplían KiyoCode con herramientas, enlaces e integraciones personalizados.
...
Esto es útil cuando desea restringir KiyoCode para que utilice únicamente proveedores específicos en lugar de deshabilitarlos uno por uno.
