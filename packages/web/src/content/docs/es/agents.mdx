...
Hay dos tipos de agentes en KiyoCode; agentes primarios y subagentes.
...
KiyoCode viene con dos agentes principales integrados, **Build** y **Plan**. Bien
...
KiyoCode viene con tres subagentes integrados, **General**, **Explore** y **Scout**. Veremos esto a continuación.

---

## Integrados

KiyoCode viene con dos agentes primarios integrados y tres subagentes integrados.
...
Un agente de solo lectura para investigar documentación externa y dependencias. Úsalo cuando necesites clonar el repositorio de una dependencia en la caché administrada de KiyoCode, inspeccionar el código fuente de una librería o contrastar el código local con implementaciones upstream sin modificar tu espacio de trabajo.
...
Si no se especifica ninguna temperatura, KiyoCode utiliza valores predeterminados específicos del modelo; normalmente 0 para la mayoría de los modelos, 0,55 para los modelos Qwen.
...
Esta ruta es relativa a donde se encuentra el archivo de configuración. Entonces esto funciona tanto para la configuración global KiyoCode como para la configuración específica del proyecto.
...
El ID del modelo en su configuración KiyoCode usa el formato `provider/model-id`. Por ejemplo, si usa [KiyoCode Zen](/docs/zen), usaría `opencode/gpt-5.1-codex` para GPT 5.1 Codex.
...
:::tip
¿Tiene un agente que le gustaría compartir? [Enviar un PR](https://github.com/dedeprogames-official/kiyocode).
:::
