...
Las herramientas permiten que LLM realice acciones en su código base. KiyoCode viene con un conjunto de herramientas integradas, pero puede ampliarlo con [herramientas personalizadas](/docs/custom-tools) o [servidores MCP](/docs/mcp-servers).
...
Aquí están todas las herramientas integradas disponibles en KiyoCode.
...
:::note
Esta herramienta solo está disponible cuando se utiliza el proveedor KiyoCode o cuando la variable de entorno `OPENCODE_ENABLE_EXA` está configurada en cualquier valor verdadero (por ejemplo, `true` o `1`).

Para habilitar al iniciar KiyoCode:
